...
uint16_t get_hw_rev(void) {
    uint16_t tmp;
    eeprom_read(KINTEX7SDR_I2C_ADDR_MBOARD, KINTEX7SDR_EE_MBOARD_REV, &tmp, sizeof(tmp));
    return tmp;
}
...
void handle_udp_fw_update_packet(struct socket_address src, struct socket_address dst,
  ...
  case KINTEX7SDR_FW_UPDATE_ID_WATS_TEH_FLASH_INFO_LOL: //query sector size, memory size so the host can mind the boundaries
    ...
    update_data_out.id = KINTEX7SDR_FW_UPDATE_ID_HERES_TEH_FLASH_INFO_OMG;
    break;

  case KINTEX7SDR_FW_UPDATE_ID_I_CAN_HAS_HW_REV_LOL: //get the hardware revision of the platform for validation checking
    update_data_out.data.hw_rev = (uint32_t) get_hw_rev();
    update_data_out.id = KINTEX7SDR_FW_UPDATE_ID_HERES_TEH_HW_REV_OMG;
    break;

  case KINTEX7SDR_FW_UPDATE_ID_ERASE_TEH_FLASHES_LOL: //out with the old
    ...
    update_data_out.id = KINTEX7SDR_FW_UPDATE_ID_ERASING_TEH_FLASHES_OMG;
    break;

  case KINTEX7SDR_FW_UPDATE_ID_R_U_DONE_ERASING_LOL:
    ...
    if(spi_flash_async_erase_poll(&spi_flash_async_state)) update_data_out.id = KINTEX7SDR_FW_UPDATE_ID_IM_DONE_ERASING_OMG;
    else update_data_out.id = KINTEX7SDR_FW_UPDATE_ID_NOPE_NOT_DONE_ERASING_OMG;
    break;

  case KINTEX7SDR_FW_UPDATE_ID_WRITE_TEH_FLASHES_LOL: //and in with the new
    ...
    update_data_out.id = KINTEX7SDR_FW_UPDATE_ID_WROTE_TEH_FLASHES_OMG;
    break;

  case KINTEX7SDR_FW_UPDATE_ID_READ_TEH_FLASHES_LOL: //for verify
    ...
    update_data_out.id = KINTEX7SDR_FW_UPDATE_ID_KK_READ_TEH_FLASHES_OMG;
    break;

  case KINTEX7SDR_FW_UPDATE_ID_RESET_MAH_COMPUTORZ_LOL: //for if we ever get the ICAP working
    //should reset via icap_reload_fpga(uint32_t flash_address);
    update_data_out.id = KINTEX7SDR_FW_UPDATE_ID_RESETTIN_TEH_COMPUTORZ_OMG;
    //you should note that if you get a reply packet to this the reset has obviously failed
    //icap_reload_fpga(0);
    warmboot(0);
    break;

//  case KINTEX7SDR_FW_UPDATE_ID_KTHXBAI: //see ya
//    break;
