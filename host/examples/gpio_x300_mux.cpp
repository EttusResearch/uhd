...
static const std::string GPIO_DEFAULT_SRC =
    ""; // empty string sets source for each bit based on ATR settings
    ...
    const size_t num_bits)
{
    const std::vector<std::string> attrs = {
        "SRC", "CTRL", "DDR", "ATR_0X", "ATR_RX", "ATR_TX", "ATR_XX", "OUT", "READBACK"};
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    // this test is specifically for the x300 series with front-panel GPIO source mux
    // control
    if (usrp->get_mboard_name(0).substr(0, 2) != "X3") {
        std::cout << "This test is only for X3xx devices with front-panel GPIO source "
                     "mux control."
                  << std::endl;
        return ~0;
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (vm.count("bitbang")) {
            ...
            while (not stop_signal_called and usrp->get_time_now() < stop_time) {
                ...
                std::cout << "\rREADBACK of " << subdev_specs[i] << ": "
                          << to_bit_string(rb, num_bits);
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
            }
            std::cout << std::endl;
        } else {
            // test user controlled GPIO and ATR idle
            std::cout << "\nTesting user controlled GPIO and ATR idle output on "
                      << subdev_specs[i] << " ..." << std::flush;
            ...
            // test ATR RX
            std::cout << "\nTesting ATR RX output on " << subdev_specs[i] << " ..."
                      << std::flush;
            ...
            // test ATR TX
            std::cout << "\nTesting ATR TX output on " << subdev_specs[i] << " ..."
                      << std::flush;
            stop_time            = usrp->get_time_now() + dwell_time;
            tx_md.start_of_burst = true;
            ...
            // test ATR XX
            std::cout << "\nTesting ATR full duplex output on " << subdev_specs[i]
                      << " ..." << std::flush;
            rx_cmd.stream_mode = uhd::stream_cmd_t::STREAM_MODE_START_CONTINUOUS;
            rx_stream->issue_stream_cmd(rx_cmd);
            tx_md.start_of_burst = true;
