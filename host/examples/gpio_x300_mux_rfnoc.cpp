...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (!vm.count("bitbang")) {
            ...
            radio_ctrl[i]->set_gpio_attr(
                fp_gpio_attrs[i], "DDR", ~0, GPIO_BIT(num_bits - 1));
        ...
        if (vm.count("bitbang")) {
            ...
            while (not stop_signal_called and radio_ctrl[i]->get_time_now() < stop_time) {
                ...
                std::cout << "\rREADBACK of " << radio_id[i] << ": "
            ...
            // test ATR RX
            std::cout << "\nTesting ATR RX output on " << radio_id[i] << " ..."
                      << std::flush;
            ...
            // test ATR TX
            std::cout << "\nTesting ATR TX output on " << radio_id[i] << " ..."
                      << std::flush;
            stop_time            = radio_ctrl[i]->get_time_now() + dwell_time;
            tx_md.start_of_burst = true;
            ...
            // test ATR XX
            std::cout << "\nTesting ATR full duplex output on " << radio_id[i] << " ..."
                      << std::flush;
            rx_cmd.stream_mode = uhd::stream_cmd_t::STREAM_MODE_START_CONTINUOUS;
            rx_stream[i]->issue_stream_cmd(rx_cmd);
            tx_md.start_of_burst = true;
