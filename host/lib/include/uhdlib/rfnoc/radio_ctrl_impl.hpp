...
#define UHD_RFNOC_RADIO_BLOCK_CONSTRUCTOR(CLASS_NAME)                  \
    CLASS_NAME##_impl::CLASS_NAME##_impl(const make_args_t& make_args) \
...
public:
    ...
    virtual double get_input_samp_rate(size_t /* port */)
    {
        return get_rate();
    }
    virtual double get_output_samp_rate(size_t /* port */)
    {
        return get_rate();
    }
    ...
    bool in_continuous_streaming_mode(const size_t chan)
    {
        return _continuous_streaming.at(chan);
    }
    void rx_ctrl_clear_cmds(const size_t port)
    {
        sr_write(regs::RX_CTRL_CLEAR_CMDS, 0, port);
    }
...
protected: // TODO see what's protected and what's private
    ...
    struct regs
    {
        ...
        static inline uint32_t rb_addr(const uint32_t offset)
        {
            return offset * 8;
        }
        ...
        static const uint32_t CLEAR_CMDS =
            131; // Any write to this reg clears the command FIFO
        ...
        // NOTE: Upper 32 registers (224-255) are reserved for the output settings bus for
        // use with
    ...
    inline bool _is_streamer_active(uhd::direction_t dir, const size_t chan) const
    {
        switch (dir) {
            case uhd::TX_DIRECTION:
                return _tx_streamer_active.count(chan) ? _tx_streamer_active.at(chan)
                                                       : false;
            case uhd::RX_DIRECTION:
                return _rx_streamer_active.count(chan) ? _rx_streamer_active.at(chan)
                                                       : false;
            case uhd::DX_DIRECTION:
                return (_tx_streamer_active.count(chan)
                           and _rx_streamer_active.count(chan))
                           ? _rx_streamer_active.at(chan) and _tx_streamer_active.at(chan)
                           : false;
            default:
                return false;
        }
