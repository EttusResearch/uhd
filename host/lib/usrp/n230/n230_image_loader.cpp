...
{
    xil_bitfile_hdr_t()
        : valid(false), userid(0), product(""), fpga(""), timestamp(""), filesize(0)
    {
    }
...
{
    return (static_cast<uint16_t>(buf[0]) << 8) | (static_cast<uint16_t>(buf[1]) << 0);
}
...
static void _parse_bitfile_header(const std::string& filepath, xil_bitfile_hdr_t& hdr)
{
    ...
    if (memcmp(buf, signature, 10) == 0) { // Validate signature
        ...
        for (size_t i = 0; i < 4; i++) {
            ...
            uint16_t len = _to_uint16(buf + ptr);
            ptr += 2;
            fields[key] = std::string(reinterpret_cast<char*>(buf + ptr), size_t(len));
            ptr += len;
        }
...
static size_t _send_and_recv(
    udp_simple::sptr xport, n230_flash_prog_t& out, n230_flash_prog_t& in)
{
...
static bool n230_image_loader(const image_loader::image_loader_args_t& loader_args)
{
    ...
    if (devs.size() == 0 or !loader_args.load_fpga)
        return false;
    if (devs.size() > 1) {
        throw uhd::runtime_error("Multiple devices match the specified args. To avoid "
                                 "accidentally updating the "
                                 "wrong device, please narrow the search by specifying a "
                                 "unique \"addr\" argument.");
    }
    ...
    if (not fpga_path_specified) {
        fpga_img_path = (fs::path(uhd::get_pkg_path()) / "share" / "uhd" / "images"
                         / "usrp_n230_fpga.bit")
                            .string();
    }
    ...
    if (not boost::filesystem::exists(fpga_img_path)) {
        if (fpga_path_specified) {
            throw uhd::runtime_error(
                str(boost::format("The file \"%s\" does not exist.") % fpga_img_path));
        } else {
            throw uhd::runtime_error(
                str(boost::format("Could not find the default FPGA image: %s.\n"
                                  "Either specify the --fpga-path argument or download "
                                  "the latest prebuilt images:\n"
                                  "%s\n")
    ...
    udp_simple::sptr udp_xport = udp_simple::make_connected(
        dev["addr"], BOOST_STRINGIZE(N230_FW_COMMS_FLASH_PROG_PORT));
    ...
    if (hdr.valid and hdr.product == "n230") {
        if (hdr.userid != 0x5AFE0000) {
            ...
            while (bytes_written < image_size) {
                size_t payload_size = std::min<size_t>(
                    image_size - bytes_written, N230_FLASH_COMM_MAX_PAYLOAD_SIZE);
                if (bytes_written % SECTOR_SIZE == 0) {
                    out.flags = htonx<uint32_t>(
                        N230_FLASH_COMM_FLAGS_ACK | N230_FLASH_COMM_CMD_ERASE_FPGA);
                    out.offset = htonx<uint32_t>(bytes_written + IMAGE_BASE);
                    out.size   = htonx<uint32_t>(payload_size);
                    _send_and_recv(udp_xport, out, in);
                }
                out.flags = htonx<uint32_t>(
                    N230_FLASH_COMM_FLAGS_ACK | N230_FLASH_COMM_CMD_WRITE_FPGA);
                out.offset = htonx<uint32_t>(bytes_written + IMAGE_BASE);
                out.size   = htonx<uint32_t>(payload_size);
                ...
                std::cout << boost::format("\r-- Loading FPGA image: %d%%")
                                 % (int(double(bytes_written) / double(image_size)
                                        * 100.0))
                          << std::flush;
            }
            std::cout << std::endl << "FPGA image loaded successfully." << std::endl;
            std::cout << std::endl
                      << "Power-cycle the device to run the image." << std::endl;
            return true;
        } else {
        ...
        throw uhd::runtime_error(str(
            boost::format("The file at path \"%s\" is not a valid USRP N230 FPGA image.")
            % fpga_img_path));
    }
}
...
UHD_STATIC_BLOCK(register_n230_image_loader)
{
    std::string recovery_instructions =
        "Aborting. Your USRP N230 device will likely boot in safe mode.\n"
