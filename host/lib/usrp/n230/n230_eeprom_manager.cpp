...
n230_eeprom_manager::n230_eeprom_manager(const std::string& addr) : _seq_num(0)
{
...
const mboard_eeprom_t& n230_eeprom_manager::read_mb_eeprom()
{
    ...
    const n230_eeprom_map_t* map_ptr =
        reinterpret_cast<const n230_eeprom_map_t*>(_response.data);
    ...
    _mb_eeprom["product"]  = std::to_string(uhd::htonx<uint16_t>(map.hw_product));
    _mb_eeprom["revision"] = std::to_string(uhd::htonx<uint16_t>(map.hw_revision));
    ...
    if (ver_major == 1 and ver_minor == 0) {
        ...
        _mb_eeprom["revision_compat"] =
            std::to_string(uhd::htonx<uint16_t>(map.hw_revision_compat));
    }
    _mb_eeprom["serial"] = _bytes_to_string(map.serial, N230_EEPROM_SERIAL_LEN);

    // Extract ethernet info
    _mb_eeprom["gateway"] =
        boost::asio::ip::address_v4(uhd::htonx<uint32_t>(map.gateway)).to_string();
    for (size_t i = 0; i < N230_MAX_NUM_ETH_PORTS; i++) {
        const std::string n(1, i + '0');
        _mb_eeprom["ip-addr" + n] =
            boost::asio::ip::address_v4(uhd::htonx<uint32_t>(map.eth_info[i].ip_addr))
                .to_string();
        _mb_eeprom["subnet" + n] =
            boost::asio::ip::address_v4(uhd::htonx<uint32_t>(map.eth_info[i].subnet))
                .to_string();
    ...
    _mb_eeprom["name"] = _bytes_to_string(map.user_name, N230_EEPROM_NAME_LEN);
...
void n230_eeprom_manager::write_mb_eeprom(const mboard_eeprom_t& eeprom)
{
    ...
    if (_mb_eeprom.has_key("product")) {
        map.hw_product =
            uhd::htonx<uint16_t>(boost::lexical_cast<uint16_t>(_mb_eeprom["product"]));
    }
    if (_mb_eeprom.has_key("revision")) {
        map.hw_revision =
            uhd::htonx<uint16_t>(boost::lexical_cast<uint16_t>(_mb_eeprom["revision"]));
    }
    ...
    for (size_t i = 0; i < N230_MAX_NUM_ETH_PORTS; i++) {
        ...
        if (_mb_eeprom.has_key("ip-addr" + n)) {
            map.eth_info[i].ip_addr = uhd::htonx<uint32_t>(
                boost::asio::ip::address_v4::from_string(_mb_eeprom["ip-addr" + n])
                    .to_ulong());
        }
        if (_mb_eeprom.has_key("subnet" + n)) {
            map.eth_info[i].subnet = uhd::htonx<uint32_t>(
                boost::asio::ip::address_v4::from_string(_mb_eeprom["subnet" + n])
                    .to_ulong());
        }
        if (_mb_eeprom.has_key("mac-addr" + n)) {
            byte_vector_t mac_addr =
                mac_addr_t::from_string(_mb_eeprom["mac-addr" + n]).to_bytes();
...
void n230_eeprom_manager::_transact(const uint32_t command)
{
    ...
    // Recv reply
    const size_t nbytes = _udp_xport->recv(
        boost::asio::buffer(&_response, sizeof(_response)), UDP_TIMEOUT_IN_SEC);
    if (nbytes == 0)
        throw uhd::io_error("n230_eeprom_manager::_transact failure");
