...
// Factory method
//----------------------------------------------------------
uhd::wb_iface::sptr n230_fw_ctrl_iface::make(uhd::transport::udp_simple::sptr udp_xport,
...
n230_fw_ctrl_iface::n230_fw_ctrl_iface(uhd::transport::udp_simple::sptr udp_xport,
    const uint16_t product_id,
    const bool verbose)
    : _product_id(product_id), _verbose(verbose), _udp_xport(udp_xport), _seq_num(0)
{
    flush();
    peek32(0);
}
    ...
    for (size_t i = 1; i <= NUM_RETRIES; i++) {
        try {
            ...
            const std::string error_msg =
                str(boost::format("udp fw poke32 failure #%u\n%s") % i % ex.what());
            if (_verbose)
                UHD_LOGGER_WARNING("N230") << error_msg;
            if (i == NUM_RETRIES)
...
uint32_t n230_fw_ctrl_iface::peek32(const wb_addr_type addr)
{
    ...
    for (size_t i = 1; i <= NUM_RETRIES; i++) {
        try {
            ...
            const std::string error_msg =
                str(boost::format("udp fw peek32 failure #%u\n%s") % i % ex.what());
            if (_verbose)
                UHD_LOGGER_WARNING("N230") << error_msg;
            if (i == NUM_RETRIES)
                throw uhd::io_error(error_msg);
...
void n230_fw_ctrl_iface::_poke32(const wb_addr_type addr, const uint32_t data)
{
    ...
    fw_comm_pkt_t reply;
    const size_t nbytes =
        _udp_xport->recv(boost::asio::buffer(&reply, sizeof(reply)), 1.0);
    if (nbytes == 0)
        throw uhd::io_error("udp fw poke32 - reply timed out");
...
uint32_t n230_fw_ctrl_iface::_peek32(const wb_addr_type addr)
{
    ...
    const size_t nbytes =
        _udp_xport->recv(boost::asio::buffer(&reply, sizeof(reply)), 1.0);
    if (nbytes == 0)
        throw uhd::io_error("udp fw peek32 - reply timed out");
...
std::vector<std::string> n230_fw_ctrl_iface::discover_devices(
    const std::string& addr_hint, const std::string& port, uint16_t product_id)
{
    ...
    try {
        ...
        UHD_LOGGER_ERROR("N230")
            << boost::format("Cannot open UDP transport on %s for discovery\n%s")
                   % addr_hint % e.what();
        return addrs;
    }
    ...
    while (true) {
        ...
        if (nbytes != sizeof(fw_comm_pkt_t))
            break; // No more responses or responses are invalid

        const fw_comm_pkt_t* reply = (const fw_comm_pkt_t*)buff;
        if (request.id == reply->id && request.flags == reply->flags
...
uint32_t n230_fw_ctrl_iface::get_iface_id(
    const std::string& addr, const std::string& port, uint16_t product_id)
{
    ...
    const fw_comm_pkt_t* reply = (const fw_comm_pkt_t*)buff;
    if (nbytes > 0 && request.id == reply->id && request.flags == reply->flags
        && request.sequence == reply->sequence) {
        return uhd::ntohx<uint32_t>(reply->data[0]);
    } else {
