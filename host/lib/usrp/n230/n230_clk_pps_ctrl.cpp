...
{
public:
    n230_clk_pps_ctrl_impl(ad9361_ctrl::sptr codec_ctrl,
    ...
    {
        UHD_LOGGER_INFO("N230")
            << "Configuring a tick rate of " << rate / 1e6 << " MHz... ";
    ...
    uhd::sensor_value_t get_ref_locked()
    {
        ...
        if (_clock_source == "external" || _clock_source == "gpsdo") {
            ...
            // If the source is internal, the charge pump on the ADF4001 is tristated
            // which means that the 40MHz VCTXXO is free running i.e. always "locked"
            locked = true;
        }
...
n230_clk_pps_ctrl::sptr n230_clk_pps_ctrl::make(ad9361_ctrl::sptr codec_ctrl,
    n230_ref_pll_ctrl::sptr ref_pll_ctrl,
    fpga::core_misc_reg_t& core_misc_reg,
    ...
    const std::vector<time_core_3000::sptr>& time_cores)
{
    return sptr(new n230_clk_pps_ctrl_impl(codec_ctrl,
        ref_pll_ctrl,
        core_misc_reg,
        core_pps_sel_reg,
        core_status_reg,
