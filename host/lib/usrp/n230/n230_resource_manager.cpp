...
n230_resource_manager::n230_resource_manager(
    const std::vector<std::string> ip_addrs, const bool safe_mode)
    : _safe_mode(safe_mode), _last_host_enpoint(0)
{
    if (_safe_mode)
        UHD_LOGGER_WARNING("N230") << "Initializing device in safe mode\n";
    ...
    for (const std::string& addr : ip_addrs) {
        ...
        try {
            iface_id = n230::n230_fw_ctrl_iface::get_iface_id(conn_iface.ip_addr,
                BOOST_STRINGIZE(N230_FW_COMMS_UDP_PORT),
                N230_FW_PRODUCT_ID);
        } catch (uhd::io_error&) {
            throw uhd::io_error(
                str(boost::format("Could not communicate with the device over address %s")
                    % conn_iface.ip_addr));
        }
        switch (iface_id) {
            case N230_ETH0_IFACE_ID:
                conn_iface.type = ETH0;
                break;
            case N230_ETH1_IFACE_ID:
                conn_iface.type = ETH1;
                break;
            default: {
                if (dual_eth_expected) {
                    throw uhd::runtime_error("N230 Initialization Error: Could not "
                                             "detect ethernet port number.");
                } else {
                    // For backwards compatibility, if only one port is specified, assume
                    // that a detection failure means that the device does not support
    ...
    if (_eth_conns.size() < 1) {
        throw uhd::runtime_error(
            "N230 Initialization Error: No eth interfaces specified.)");
    }

    // Create firmware communication interface
    _fw_ctrl = n230::n230_fw_ctrl_iface::make(
        transport::udp_simple::make_connected(
            _get_conn(PRI_ETH).ip_addr, BOOST_STRINGIZE(N230_FW_COMMS_UDP_PORT)),
        N230_FW_PRODUCT_ID);
    if (_fw_ctrl.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create n230_ctrl_iface.)");
    }
    _check_fw_compat();

    // Start the device claimer
    _claimer_task =
        uhd::task::make(boost::bind(&n230_resource_manager::_claimer_loop, this));
    ...
    transport::zero_copy_if::sptr core_xport = _create_transport(
        _get_conn(PRI_ETH), core_sid, device_addr_t(), dummy_out_params);
    if (core_xport.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create settings transport.)");
    }
    _core_ctrl = radio_ctrl_core_3000::make(
        fpga::CVITA_BIG_ENDIAN, core_xport, core_xport, core_sid.get());
    if (_core_ctrl.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create settings ctrl.)");
    }
    _check_fpga_compat();

    UHD_LOGGER_INFO("N230") << boost::format(
                                   "Version signatures... Firmware:%s FPGA:%s...")
                                   % _fw_version.get_hash_str()
                                   % _fpga_version.get_hash_str();
    ...
    if (_core_spi_ctrl.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create SPI ctrl.)");
    }

    // Create AD9361 interface
    UHD_LOGGER_INFO("N230") << "Initializing CODEC...";
    _codec_ctrl = ad9361_ctrl::make_spi(boost::make_shared<n230_ad9361_client_t>(),
        _core_spi_ctrl,
        fpga::AD9361_SPI_SLAVE_NUM);
    if (_codec_ctrl.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create Catalina ctrl.)");
    }
    ...
    if (_ref_pll_ctrl.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create ADF4001 ctrl.)");
    }
    ...
    _clk_pps_ctrl = n230_clk_pps_ctrl::make(_codec_ctrl,
        _ref_pll_ctrl,
        _core_misc_reg,
        _core_pps_sel_reg,
        _core_status_reg,
    ...
    if (_clk_pps_ctrl.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create clock and PPS ctrl.)");
    }

    // Create front-end control interface
    _frontend_ctrl =
        n230_frontend_ctrl::make(_core_ctrl, _core_misc_reg, _codec_ctrl, gpio_cores);
    if (_frontend_ctrl.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create front-end ctrl.)");
    }

    // Create miniSAS GPIO interfaces
    _ms0_gpio = gpio_atr::gpio_atr_3000::make(_core_ctrl,
        fpga::sr_addr(fpga::SR_CORE_MS0_GPIO),
        fpga::rb_addr(fpga::RB_CORE_MS0_GPIO));
    _ms0_gpio->set_atr_mode(gpio_atr::MODE_GPIO, gpio_atr::gpio_atr_3000::MASK_SET_ALL);
    _ms1_gpio = gpio_atr::gpio_atr_3000::make(_core_ctrl,
        fpga::sr_addr(fpga::SR_CORE_MS1_GPIO),
        fpga::rb_addr(fpga::RB_CORE_MS1_GPIO));
    _ms1_gpio->set_atr_mode(gpio_atr::MODE_GPIO, gpio_atr::gpio_atr_3000::MASK_SET_ALL);

    // Create GPSDO interface
    if (_core_status_reg.read(fpga::core_status_reg_t::GPSDO_STATUS)
        != fpga::GPSDO_ST_ABSENT) {
        UHD_LOGGER_INFO("N230") << "Detecting GPSDO.... ";
        try {
            const sid_t gps_uart_sid = _generate_sid(GPS_UART, _get_conn(PRI_ETH).type);
            transport::zero_copy_if::sptr gps_uart_xport = _create_transport(
                _get_conn(PRI_ETH), gps_uart_sid, device_addr_t(), dummy_out_params);
            ...
            std::this_thread::sleep_for(
                std::chrono::seconds(1)); // allow for a little propagation
            _gps_ctrl = gps_ctrl::make(_gps_uart);
        } catch (std::exception& e) {
            UHD_LOGGER_ERROR("N230")
                << "An error occurred making GPSDO control: " << e.what();
        }
        if (not is_gpsdo_present()) {
            _core_ctrl->poke32(
                fpga::sr_addr(fpga::SR_CORE_GPSDO_ST), fpga::GPSDO_ST_ABSENT);
        }
    }
    ...
    for (size_t i = 0; i < fpga::NUM_RADIOS; i++) {
        ...
        if (!radio_selftest_pass) {
            throw uhd::runtime_error(
                "N230 Initialization Error: Data loopback test failed.)");
        }
        ...
        if (!codec_selftest_pass) {
            throw uhd::runtime_error(
                "N230 Initialization Error: Codec loopback test failed.)");
        }
    }
    ...
    transport::udp_zero_copy::buff_params& buff_out_params)
{
    ...
    const sid_t temp_sid_pair   = _generate_sid(
        direction == RX_DATA ? RADIO_RX_DATA : RADIO_TX_DATA, conn.type, radio_instance);
    transport::zero_copy_if::sptr xport =
        _create_transport(conn, temp_sid_pair, params, buff_out_params);
    if (xport.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Create Data Transport: Could not create data transport.)");
    } else {
...
void n230_resource_manager::_claimer_loop()
{
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(N230_CLAIMER_TIMEOUT_IN_MS / 2));
}
...
void n230_resource_manager::_initialize_radio(size_t instance)
{
    ...
    // Create common settings interface
    const sid_t ctrl_sid =
        _generate_sid(RADIO_CONTROL, _get_conn(PRI_ETH).type, instance);
    transport::udp_zero_copy::buff_params buff_out_params;
    ...
    if (!reg_selftest_pass) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Register loopback test failed.)");
    }

    // Write-only ATR interface
    radio.gpio_atr = gpio_atr::gpio_atr_3000::make_write_only(
        radio.ctrl, fpga::sr_addr(fpga::SR_RADIO_ATR));
    radio.gpio_atr->set_atr_mode(
        gpio_atr::MODE_ATR, gpio_atr::gpio_atr_3000::MASK_SET_ALL);
    ...
    radio.time =
        time_core_3000::make(radio.ctrl, fpga::sr_addr(fpga::SR_RADIO_TIME), time_bases);
    if (radio.time.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create time core.)");
    }

    // RX DSP
    radio.framer =
        rx_vita_core_3000::make(radio.ctrl, fpga::sr_addr(fpga::SR_RADIO_RX_CTRL));
    radio.ddc = rx_dsp_core_3000::make(
        radio.ctrl, fpga::sr_addr(fpga::SR_RADIO_RX_DSP), true /*old DDC?*/);
    if (radio.framer.get() == NULL || radio.ddc.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create RX DSP interface.)");
    }
    radio.ddc->set_link_rate(fpga::N230_LINK_RATE_BPS);

    // TX DSP
    radio.deframer =
        tx_vita_core_3000::make(radio.ctrl, fpga::sr_addr(fpga::SR_RADIO_TX_CTRL));
    ...
    radio.user_settings = user_settings_core_3000::make(radio.ctrl,
        fpga::sr_addr(fpga::SR_RADIO_USER_SR),
        fpga::rb_addr(fpga::SR_RADIO_USER_RB));
    if (radio.user_settings.get() == NULL) {
        throw uhd::runtime_error(
            "N230 Initialization Error: Could not create user settings bus.)");
    }
}
...
const sid_t n230_resource_manager::_generate_sid(
    const n230_endpoint_t type, const n230_eth_port_t xport, size_t instance)
{
    fpga::xb_endpoint_t xb_dest_ep;
    uint8_t sid_dest_ep           = 0;
    fpga::xb_endpoint_t xb_ret_ep = (xport == ETH1) ? fpga::N230_XB_DST_E1
                                                    : fpga::N230_XB_DST_E0;
    uint8_t sid_ret_addr = (xport == ETH1) ? N230_HOST_SRC_ADDR_ETH1
                                           : N230_HOST_SRC_ADDR_ETH0;
    ...
    _fw_ctrl->poke32(
        fw::reg_addr(fw::WB_SBRB_BASE, fw::SR_ZPU_XB_LOCAL), sid.get_dst_addr());
    ...
    _fw_ctrl->poke32(fw::reg_addr(fw::WB_XB_SBRB_BASE, sid.get_src_addr()),
        static_cast<uint32_t>(xb_ret_ep));
    // Program CAM entry for outgoing packets matching a N230 resource (for example a
    // Radio) This type of packet does matches the XB_LOCAL address and is looked up in
    // the upper half of the CAM
    _fw_ctrl->poke32(fw::reg_addr(fw::WB_XB_SBRB_BASE, 256 + sid.get_dst_endpoint()),
        static_cast<uint32_t>(xb_dest_ep));
...
transport::zero_copy_if::sptr n230_resource_manager::_create_transport(
    ...
    const sid_t& sid,
    const device_addr_t& buff_params,
    transport::udp_zero_copy::buff_params& buff_params_out)
{
    ...
    transport::zero_copy_if::sptr xport = transport::udp_zero_copy::make(eth_conn.ip_addr,
        std::to_string(fpga::CVITA_UDP_PORT),
        default_buff_args,
        buff_params_out,
        buff_params);
    ...
    transport::zero_copy_if& xport, const n230_eth_port_t port, const sid_t& sid)
{
    ...
    uint32_t disp_base_offset = ((port == ETH1) ? fw::SR_ZPU_ETHINT1 : fw::SR_ZPU_ETHINT0)
                                + fw::SR_ZPU_ETHINT_DISPATCHER_BASE;
    _fw_ctrl->poke32(
        fw::reg_addr(fw::WB_SBRB_BASE, disp_base_offset + fw::ETH_FRAMER_SRC_UDP_PORT),
        fpga::CVITA_UDP_PORT);
...
bool n230_resource_manager::_radio_data_loopback_self_test(wb_iface::sptr iface)
{
    ...
    for (size_t i = 0; i < 100; i++) {
        ...
        iface->peek64(
            fpga::rb_addr(fpga::RB_RADIO_CODEC_DATA)); // block until request completes
        std::this_thread::sleep_for(std::chrono::microseconds(
            100)); // wait for loopback to propagate through codec
        const uint64_t rb_word64 =
            iface->peek64(fpga::rb_addr(fpga::RB_RADIO_CODEC_DATA));
        const uint32_t rb_tx = uint32_t(rb_word64 >> 32);
        const uint32_t rb_rx = uint32_t(rb_word64 & 0xffffffff);
        test_fail            = word32 != rb_tx or word32 != rb_rx;
        if (test_fail) {
            UHD_LOG_ERROR("N230",
                str(boost::format("mismatch (exp:%x, got:%x and %x)... ") % word32 % rb_tx
                    % rb_rx));
            break; // exit loop on any failure
        }
    }
...
std::string n230_resource_manager::_get_fpga_upgrade_msg()
{
    ...
    return str(boost::format(
               ...
               % print_utility_error("uhd_images_downloader.py") % img_loader_path
               % _get_conn(PRI_ETH).ip_addr);
}
...
void n230_resource_manager::_check_fw_compat()
{
    ...
    _fw_version.version_hash =
        _fw_ctrl->peek32(N230_FW_HOST_SHMEM_OFFSET(fw_version_hash));
    ...
    if (_fw_version.compat_major != N230_FW_COMPAT_NUM_MAJOR) {
        throw uhd::runtime_error(str(
            ...
            % static_cast<uint32_t>(_fw_version.compat_minor) % _get_fpga_upgrade_msg()));
    }
}
...
void n230_resource_manager::_check_fpga_compat()
{
    ...
    const uint64_t version_hash =
        _core_ctrl->peek64(fpga::rb_addr(fpga::RB_CORE_VERSION_HASH));
    ...
    if (is_safe_image && !_safe_mode) {
        throw uhd::runtime_error(
            ...
            "This could have happened because the production FPGA image in the flash was "
            "either corrupt or non-existent\n"
            "To remedy this error, please burn a valid FPGA image to the flash.\n"
            "To continue using the failsafe image with UHD, create the UHD device with "
            "the \"safe_mode\" device arg.\n"
            "Radio functionality/performance not guaranteed when operating in safe "
            "mode.\n");
    } else if (_fpga_version.compat_major != fpga::RB_N230_COMPAT_MAJOR
               && !is_safe_image) {
        throw uhd::runtime_error(str(
            boost::format("Expected FPGA compatibility number %d.x, but got %d.%d:\n"
