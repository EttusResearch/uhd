...
uhd::device_addrs_t n230_impl::n230_find(const uhd::device_addr_t& multi_dev_hint)
{
    ...
    if (hint.has_key("type") and hint["type"] != "n230")
        return n230_addrs;
    ...
    if (not hint.has_key("addr")) {
        for (const if_addrs_t& if_addrs : get_if_addrs()) {
            ...
            n230_addrs.insert(
                n230_addrs.begin(), new_n230_addrs.begin(), new_n230_addrs.end());
        }
        return n230_addrs;
    }

    std::vector<std::string> discovered_addrs = n230_fw_ctrl_iface::discover_devices(
    ...
    for (const std::string& addr : discovered_addrs) {
        ...
        udp_simple::sptr ctrl_xport = udp_simple::make_connected(
            new_addr["addr"], BOOST_STRINGIZE(N230_FW_COMMS_UDP_PORT));
        ...
        while (first_conn_retries > 0) {
            try {
                fw_ctrl = n230_fw_ctrl_iface::make(
                    ctrl_xport, N230_FW_PRODUCT_ID, false /*verbose*/);
                break;
        ...
        if (first_conn_retries > 0) {
            uint32_t compat_reg =
                fw_ctrl->peek32(fw::reg_addr(fw::WB_SBRB_BASE, fw::RB_ZPU_COMPAT));
            if (fw::get_prod_num(compat_reg) == fw::PRODUCT_NUM) {
                if (!n230_resource_manager::is_device_claimed(fw_ctrl)) {
                    ...
                    try {
                        ...
                        // set these values as empty string so the device may still be
                        // found and the filter's below can still operate on the
                        // discovered device
                        new_addr["name"]   = "";
                        new_addr["serial"] = "";
                    }
...
n230_impl::n230_impl(const uhd::device_addr_t& dev_addr)
{
    ...
    if (not recover_mb_eeprom) {
        if (hw_rev_compat > N230_HW_REVISION_COMPAT) {
            throw uhd::runtime_error(
                str(boost::format("Hardware is too new for this software. Please upgrade "
    ...
    _resource_mgr =
        boost::make_shared<n230_resource_manager>(ip_addrs, _dev_args.get_safe_mode());
    _stream_mgr =
        boost::make_shared<n230_stream_manager>(_dev_args, _resource_mgr, _tree);
...
void n230_impl::_initialize_property_tree(const fs_path& mb_path)
{
    ...
    _tree->create<std::string>(mb_path / "fw_version")
        .set(str(boost::format("%u.%u")
                 % _resource_mgr->get_version(FIRMWARE, COMPAT_MAJOR)
                 % _resource_mgr->get_version(FIRMWARE, COMPAT_MINOR)));
    _tree->create<std::string>(mb_path / "fw_version_hash")
        .set(str(boost::format("%s") % _resource_mgr->get_version_hash(FIRMWARE)));
    _tree->create<std::string>(mb_path / "fpga_version")
        .set(str(boost::format("%u.%u") % _resource_mgr->get_version(FPGA, COMPAT_MAJOR)
                 % _resource_mgr->get_version(FPGA, COMPAT_MINOR)));
    _tree->create<std::string>(mb_path / "fpga_version_hash")
        .set(str(boost::format("%s") % _resource_mgr->get_version_hash(FPGA)));

    _tree->create<double>(mb_path / "link_max_rate")
        .set(_resource_mgr->get_max_link_rate());
    ...
    _tree->create<mboard_eeprom_t>(mb_path / "eeprom")
        ...
        .add_coerced_subscriber(boost::bind(&n230_eeprom_manager::write_mb_eeprom,
            _eeprom_mgr,
            _1)); //..then enable writer

    //------------------------------------------------------------------
    // Create codec nodes
    //------------------------------------------------------------------
    const fs_path rx_codec_path = mb_path / ("rx_codecs") / "A";
    _tree->create<std::string>(rx_codec_path / "name").set("N230 RX dual ADC");
    _tree->create<int>(rx_codec_path / "gains"); // Empty because gains are in frontend

    const fs_path tx_codec_path = mb_path / ("tx_codecs") / "A";
    _tree->create<std::string>(tx_codec_path / "name").set("N230 TX dual DAC");
    ...
    _tree->create<double>(mb_path / "tick_rate")
        .set_coercer(boost::bind(&n230_clk_pps_ctrl::set_tick_rate,
            _resource_mgr->get_clk_pps_ctrl_sptr(),
            _1))
        .set_publisher(boost::bind(
            &n230_clk_pps_ctrl::get_tick_rate, _resource_mgr->get_clk_pps_ctrl_sptr()))
        .add_coerced_subscriber(
            boost::bind(&n230_stream_manager::update_tick_rate, _stream_mgr, _1));
    ...
    _tree->create<time_spec_t>(mb_path / "time" / "now")
        .set_publisher(
            boost::bind(&time_core_3000::get_time_now, _resource_mgr->get_radio(0).time));
    _tree->create<time_spec_t>(mb_path / "time" / "pps")
        .set_publisher(boost::bind(
            &time_core_3000::get_time_last_pps, _resource_mgr->get_radio(0).time));

    // Setup time source props
    _tree->create<std::string>(mb_path / "time_source" / "value")
        .add_coerced_subscriber(boost::bind(&n230_impl::_check_time_source, this, _1))
        .add_coerced_subscriber(boost::bind(&n230_clk_pps_ctrl::set_pps_source,
            _resource_mgr->get_clk_pps_ctrl_sptr(),
            _1))
    ...
    _tree->create<std::string>(mb_path / "clock_source" / "value")
        ...
        .add_coerced_subscriber(boost::bind(&n230_clk_pps_ctrl::set_clock_source,
            _resource_mgr->get_clk_pps_ctrl_sptr(),
            _1))
    ...
    _tree->create<sensor_value_t>(mb_path / "sensors" / "ref_locked")
        .set_publisher(boost::bind(
            &n230_clk_pps_ctrl::get_ref_locked, _resource_mgr->get_clk_pps_ctrl_sptr()));
    ...
    _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec")
        .set(subdev_spec_t())
        .add_coerced_subscriber(
            boost::bind(&n230_impl::_update_rx_subdev_spec, this, _1));
    _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec")
        .set(subdev_spec_t())
        .add_coerced_subscriber(
            boost::bind(&n230_impl::_update_tx_subdev_spec, this, _1));
        ...
        switch (attr.first) {
            ...
            case usrp::gpio_atr::GPIO_DDR:
                _tree
                    ...
                    .add_coerced_subscriber([this, attr](
                        ...
                        _resource_mgr->get_minisas_gpio_ctrl_sptr(0)->set_gpio_attr(
                            attr.first, val);
                    });
                _tree
                    ->create<std::vector<std::string>>(
                    ...
                    .add_coerced_subscriber([this, attr](
                        ...
                        _resource_mgr->get_minisas_gpio_ctrl_sptr(1)->set_gpio_attr(
                            attr.first, val);
                    });
                break;
            ...
            case usrp::gpio_atr::GPIO_READBACK:
                _tree->create<uint32_t>(mb_path / "gpio" / "FP0" / "READBACK")
                    .set_publisher(boost::bind(&gpio_atr::gpio_atr_3000::read_gpio,
                        _resource_mgr->get_minisas_gpio_ctrl_sptr(0)));
                _tree->create<uint32_t>(mb_path / "gpio" / "FP1" / "READBACK")
                    .set_publisher(boost::bind(&gpio_atr::gpio_atr_3000::read_gpio,
                        _resource_mgr->get_minisas_gpio_ctrl_sptr(1)));
                break;
            default:
                _tree->create<uint32_t>(mb_path / "gpio" / "FP0" / attr.second).set(0);
                _tree->create<uint32_t>(mb_path / "gpio" / "FP1" / attr.second).set(0);
                break;
        }
    }
    //------------------------------------------------------------------
...
void n230_impl::_initialize_radio_properties(const fs_path& mb_path, size_t instance)
{
    ...
    _tree->access<time_spec_t>(mb_path / "time" / "now")
        .add_coerced_subscriber(
            boost::bind(&time_core_3000::set_time_now, perif.time, _1));
    _tree->access<time_spec_t>(mb_path / "time" / "pps")
        .add_coerced_subscriber(
            boost::bind(&time_core_3000::set_time_next_pps, perif.time, _1));
    ...
    _tree->access<double>(mb_path / "tick_rate")
        ...
        .add_coerced_subscriber(
            boost::bind(&rx_dsp_core_3000::set_tick_rate, perif.ddc, _1));
    ...
    _tree->create<double>(rx_dsp_path / "rate" / "value")
        ...
        .add_coerced_subscriber(boost::bind(
            &n230_stream_manager::update_rx_samp_rate, _stream_mgr, instance, _1))
    ...
    _tree->access<double>(mb_path / "tick_rate")
        .add_coerced_subscriber(
            boost::bind(&tx_dsp_core_3000::set_tick_rate, perif.duc, _1));
    ...
    _tree->create<double>(tx_dsp_path / "rate" / "value")
        ...
        .add_coerced_subscriber(boost::bind(
            &n230_stream_manager::update_tx_samp_rate, _stream_mgr, instance, _1))
    ...
    for (direction_t direction : std::vector<direction_t>{RX_DIRECTION, TX_DIRECTION}) {
        ...
        const std::string key =
            boost::to_upper_copy(dir_str) + str(boost::format("%u") % (instance + 1));
        const fs_path rf_fe_path = mb_path / "dboards" / "A" / (dir_str + "_frontends")
                                   / ((instance == 0) ? "A" : "B");

        // CODEC subtree
        _resource_mgr->get_codec_mgr().populate_frontend_subtree(
            _tree->subtree(rf_fe_path), key, direction);
        ...
        if (key[0] == 'R') {
            ...
            _tree->create<std::string>(rf_fe_path / "antenna" / "value")
                .add_coerced_subscriber(boost::bind(&n230_frontend_ctrl::set_antenna_sel,
                    _resource_mgr->get_frontend_ctrl_sptr(),
                    instance,
                    _1))
                .set("RX2");
        }
