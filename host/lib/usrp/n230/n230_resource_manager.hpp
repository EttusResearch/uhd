...
enum n230_eth_port_t { ETH0, ETH1 };

enum n230_eth_pref_t { PRI_ETH, SEC_ETH };

enum n230_endpoint_t { RADIO_TX_DATA, RADIO_RX_DATA, RADIO_CONTROL, CORE, GPS_UART };

enum n230_ver_src_t { SOFTWARE, FIRMWARE, FPGA };

enum n230_version_t { COMPAT_MAJOR, COMPAT_MINOR };

enum n230_data_dir_t { RX_DATA, TX_DATA };
...
public: // Methods
    ...
    inline uint32_t get_version(n230_ver_src_t src, n230_version_t type)
    {
        switch (src) {
            case FPGA:
                return _fpga_version.get(type);
            case FIRMWARE:
                return _fw_version.get(type);
            default:
                return 0;
        }
    }
    ...
    inline const std::string get_version_hash(n230_ver_src_t src)
    {
        switch (src) {
            case FPGA:
                return _fpga_version.get_hash_str();
            case FIRMWARE:
                return _fw_version.get_hash_str();
            default:
                return "";
        }
    }
    ...
    transport::zero_copy_if::sptr create_transport(n230_data_dir_t direction,
...
private:
    struct ver_info_t
    {
        ...
        {
            return (str(boost::format("%07x%s") % (version_hash & 0x0FFFFFFF)
    ...
    transport::zero_copy_if::sptr _create_transport(const n230_eth_conn_t& eth_conn,
        const sid_t& sid,
    ...
    static boost::mutex
        _claimer_mutex; // All claims and checks in this process are serialized
