...
static const double N230_RX_SW_BUFF_FULL_FACTOR =
    0.90; // Buffer should ideally be 90% full.
 ...
 * Receive streamer
 **********************************************************************/
n230_stream_manager::n230_stream_manager(const n230_device_args_t& dev_args,
    boost::shared_ptr<n230_resource_manager> resource_mgr,
    boost::weak_ptr<property_tree> prop_tree)
    : _dev_args(dev_args), _resource_mgr(resource_mgr), _tree(prop_tree)
{
...
rx_streamer::sptr n230_stream_manager::get_rx_stream(const uhd::stream_args_t& args_)
{
    ...
    for (size_t stream_i = 0; stream_i < args.channels.size(); stream_i++) {
        ...
        if (not device_addr.has_key("recv_buff_size")) {
            device_addr["recv_buff_size"] =
                std::to_string(_dev_args.get_recv_buff_size());
        }
        if (not device_addr.has_key("recv_frame_size")) {
            device_addr["recv_frame_size"] =
                std::to_string(_dev_args.get_recv_frame_size());
        }
        if (not device_addr.has_key("num_recv_frames")) {
            device_addr["num_recv_frames"] =
                std::to_string(_dev_args.get_num_recv_frames());
        }
        ...
        spp              = std::min<size_t>(
            N230_TX_MAX_SPP, spp); // FPGA FIFO maximum for framing at full rate
        ...
        my_streamer->set_xport_chan_get_buff(
            stream_i, boost::bind(&zero_copy_if::get_recv_buff, xport, _1), true /*flush*/
        );

        my_streamer->set_overflow_handler(
            stream_i, boost::bind(&n230_stream_manager::_handle_overflow, this, chan));
        ...
        const size_t fc_handle_window =
            std::max<size_t>(1, fc_window / N230_RX_FC_REQUEST_FREQ);
        ...
        my_streamer->set_xport_handle_flowctrl(stream_i,
            boost::bind(&n230_stream_manager::_handle_rx_flowctrl,
                sid.get(),
                xport,
                fc_cache,
                _1),
        ...
        // Store a weak pointer to prevent a streamer->manager->streamer circular
        // dependency
        ...
        if (prop_tree) {
            ...
            prop_tree
                ->access<double>(
                    mb_path / "rx_dsps" / std::to_string(chan) / "rate" / "value")
                .update();
        }
    }
...
tx_streamer::sptr n230_stream_manager::get_tx_stream(const uhd::stream_args_t& args_)
{
    ...
    boost::shared_ptr<async_md_queue_t> async_md(
        new async_md_queue_t(N230_TX_MAX_ASYNC_MESSAGES));
    ...
    for (size_t stream_i = 0; stream_i < args.channels.size(); stream_i++) {
        ...
        if (not device_addr.has_key("send_buff_size")) {
            device_addr["send_buff_size"] =
                std::to_string(_dev_args.get_send_buff_size());
        }
        if (not device_addr.has_key("send_frame_size")) {
            device_addr["send_frame_size"] =
                std::to_string(_dev_args.get_send_frame_size());
        }
        if (not device_addr.has_key("num_send_frames")) {
            device_addr["num_send_frames"] =
                std::to_string(_dev_args.get_num_send_frames());
        }
        ...
        // calculate packet size
        static const size_t hdr_size =
            0 + vrt::num_vrl_words32 * sizeof(uint32_t)
            + vrt::max_if_hdr_words32 * sizeof(uint32_t)
        ...
        // flow control setup
        size_t fc_window = _get_tx_flow_control_window(bpp,
            device_addr.cast<size_t>("send_buff_size", _dev_args.get_send_buff_size()));
        // In packets
        ...
        tick_rate_retriever_t get_tick_rate_fn =
            boost::bind(&n230_stream_manager::_get_tick_rate, this);
        task::sptr task =
            task::make(boost::bind(&n230_stream_manager::_handle_tx_async_msgs,
                fc_cache,
                xport,
                get_tick_rate_fn));
        ...
        // xport.send (sptr) is required to add streamer->data-transport lifetime
        // dependency task (sptr) is required to add  a streamer->async-handler lifetime
        // dependency
        my_streamer->set_xport_chan_get_buff(stream_i,
            boost::bind(&n230_stream_manager::_get_tx_buff_with_flowctrl,
                task,
                fc_cache,
                xport,
                fc_window,
        ...
        if (prop_tree) {
            ...
            prop_tree
                ->access<double>(
                    mb_path / "tx_dsps" / std::to_string(chan) / "rate" / "value")
                .update();
        }
    }
    update_stream_states();
...
void n230_stream_manager::update_tick_rate(const double rate)
{
    for (size_t i = 0; i < fpga::NUM_RADIOS; i++) {
        ...
        if (my_rx_streamer)
            my_rx_streamer->set_tick_rate(rate);
        ...
        if (my_tx_streamer)
            my_tx_streamer->set_tick_rate(rate);
    }
}
...
void n230_stream_manager::update_stream_states()
{
    ...
    if (enb_tx0 && enb_rx0)
        fe0_state = TXRX_STREAMING;
    else if (enb_tx0)
        fe0_state = TX_STREAMING;
    else if (enb_rx0)
        fe0_state = RX_STREAMING;

    fe_state_t fe1_state = NONE_STREAMING;
    if (enb_tx1 && enb_rx1)
        fe1_state = TXRX_STREAMING;
    else if (enb_tx1)
        fe1_state = TX_STREAMING;
    else if (enb_rx1)
        fe1_state = RX_STREAMING;

    _resource_mgr->get_frontend_ctrl().set_stream_state(fe0_state, fe1_state);
}

size_t n230_stream_manager::_get_rx_flow_control_window(
...
void n230_stream_manager::_handle_overflow(const size_t i)
{
    ...
    if (my_streamer->get_num_channels() == 2) {
        ...
        const bool in_continuous_streaming_mode =
            _resource_mgr->get_radio(i).framer->in_continuous_streaming_mode();
        // stop streaming
        ...
        if (in_continuous_streaming_mode) {
            ...
            stream_cmd.time_spec =
                _resource_mgr->get_radio(i).time->get_time_now() + time_spec_t(0.01);
...
void n230_stream_manager::_handle_rx_flowctrl(const sid_t& sid,
    ...
    const size_t last_seq)
{
    ...
    // load payload
    pkt[packet_info.num_header_words32 + RXFC_CMD_CODE_OFFSET] =
        uhd::htonx<uint32_t>(N230_EVENT_CODE_FLOW_CTRL);
    pkt[packet_info.num_header_words32 + RXFC_SEQ_NUM_OFFSET] =
        uhd::htonx<uint32_t>(seq_sw);

    // send the buffer over the interface
    buff->commit(sizeof(uint32_t) * (packet_info.num_packet_words32));
}

void n230_stream_manager::_handle_tx_async_msgs(boost::shared_ptr<tx_fc_cache_t> fc_cache,
    zero_copy_if::sptr xport,
    ...
    const double timeout)
{
    while (true) {
        const size_t delta = (fc_cache->last_seq_out & HW_SEQ_NUM_MASK)
                             - (fc_cache->last_seq_ack & HW_SEQ_NUM_MASK);
        if ((delta & HW_SEQ_NUM_MASK) <= fc_pkt_window)
            break;
...
size_t n230_stream_manager::_get_tx_flow_control_window(
    size_t payload_size, size_t hw_buff_size)
{
