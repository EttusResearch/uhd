...
static const uint32_t STATE_RX_RX2  = (SFDX_RX | SFDX_TX | LED_RX);
static const uint32_t STATE_RX_TXRX = (SRX_RX | SRX_TX | LED_TXRX_RX);
static const uint32_t STATE_FDX_TXRX =
    (TX_ENABLE | SFDX_RX | SFDX_TX | LED_TXRX_TX | LED_RX);
static const uint32_t STATE_TX_TXRX = (TX_ENABLE | SFDX_RX | SFDX_TX | LED_TXRX_TX);
...
{
public:
    n230_frontend_ctrl_impl(radio_ctrl_core_3000::sptr core_ctrl,
    ...
    void set_stream_state(const fe_state_t fe0_state_, const fe_state_t fe1_state_)
    {
        ...
        const fe_state_t fe1_state = (_gpio_cores.size() > 1) ? _fe_states[1].state
                                                              : NONE_STREAMING;
        ...
        if ((num_rx + num_tx) == 0) {
            ...
            _codec_ctrl->set_active_chains(_is_tx(fe0_state),
                _is_tx(fe1_state),
                _is_rx(fe0_state),
                _is_rx(fe1_state));
        }
...
private:
    void _flush_atr_state()
    {
        for (size_t i = 0; i < _gpio_cores.size(); i++) {
            ...
            const size_t rxonly = (enb_rx) ? ((is_rx2) ? STATE_RX_RX2 : STATE_RX_TXRX)
                                           : STATE_OFF;
...
n230_frontend_ctrl::sptr n230_frontend_ctrl::make(radio_ctrl_core_3000::sptr core_ctrl,
    fpga::core_misc_reg_t& core_misc_reg,
    ad9361_ctrl::sptr codec_ctrl,
    const std::vector<gpio_atr::gpio_atr_3000::sptr>& gpio_cores)
{
    return sptr(
        new n230_frontend_ctrl_impl(core_ctrl, core_misc_reg, codec_ctrl, gpio_cores));
}
