...
public:
    ...
    n230_device_args_t()
        ...
        , _loopback_mode("loopback_mode",
              LOOPBACK_OFF,
              {{"off", LOOPBACK_OFF},
                  {"radio", LOOPBACK_RADIO},
                  {"codec", LOOPBACK_CODEC}})
    {
    }
    ...
    {
        return _master_clock_rate.to_string() + ", " + _send_frame_size.to_string() + ", "
               + _recv_frame_size.to_string() + ", " + _num_send_frames.to_string() + ", "
               + _num_recv_frames.to_string() + ", " + _send_buff_size.to_string() + ", "
               + _recv_buff_size.to_string() + ", " + _safe_mode.to_string() + ", "
...
private:
    virtual void _parse(const device_addr_t& dev_args)
    {
        ...
        if (dev_args.has_key(_loopback_mode.key()))
            _loopback_mode.parse(
                dev_args[_loopback_mode.key()], false /* assert invalid */);
