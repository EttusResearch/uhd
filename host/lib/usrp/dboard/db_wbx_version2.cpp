...
wbx_base::wbx_version2::wbx_version2(wbx_base* _self_wbx_base)
{
    ...
    this->get_rx_subtree()
        ...
        .set_coercer(boost::bind(
            &wbx_base::wbx_version2::set_lo_freq, this, dboard_iface::UNIT_RX, _1))
    ...
    for (const std::string& name : wbx_v2_tx_gain_ranges.keys()) {
        self_base->get_tx_subtree()
            ...
            .set_coercer(
                boost::bind(&wbx_base::wbx_version2::set_tx_gain, this, _1, name))
    ...
    this->get_tx_subtree()
        ->create<double>("freq/value")
        .set_coercer(boost::bind(
            &wbx_base::wbx_version2::set_lo_freq, this, dboard_iface::UNIT_TX, _1))
        .set((wbx_v2_freq_range.start() + wbx_v2_freq_range.stop()) / 2.0);
    this->get_tx_subtree()->create<meta_range_t>("freq/range").set(wbx_v2_freq_range);
    this->get_tx_subtree()
        ->create<bool>("enabled")
        .add_coerced_subscriber(
            boost::bind(&wbx_base::wbx_version2::set_tx_enabled, this, _1))
