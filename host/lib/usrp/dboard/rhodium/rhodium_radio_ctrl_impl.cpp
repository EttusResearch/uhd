...
device_addr_t _get_tune_args(
    uhd::property_tree::sptr tree, std::string _radio_slot, uhd::direction_t dir)
{
...
void rhodium_radio_ctrl_impl::set_tx_antenna(const std::string& ant, const size_t chan)
{
...
void rhodium_radio_ctrl_impl::set_rx_antenna(const std::string& ant, const size_t chan)
{
...
double rhodium_radio_ctrl_impl::set_tx_frequency(const double freq, const size_t chan)
{
    ...
    if (freq != coerced_target_freq) {
        UHD_LOG_DEBUG(unique_id(),
            "Requested frequency is outside supported range. Coercing to "
                << coerced_target_freq);
    }
...
double rhodium_radio_ctrl_impl::set_rx_frequency(const double freq, const size_t chan)
{
...
double rhodium_radio_ctrl_impl::set_tx_gain(const double gain, const size_t chan)
{
...
double rhodium_radio_ctrl_impl::set_rx_gain(const double gain, const size_t chan)
{
...
void rhodium_radio_ctrl_impl::_identify_with_leds(double identify_duration)
{
...
void rhodium_radio_ctrl_impl::_update_atr(const std::string& ant, const direction_t dir)
{
...
bool rhodium_radio_ctrl_impl::_get_spur_dodging_enabled(uhd::direction_t dir) const
{
    ...
    auto block_value =
        _tree->access<std::string>(get_arg_path(SPUR_DODGING_ARG_NAME) / "value").get();
    ...
    std::string spur_dodging_arg =
        dict.cast<std::string>(SPUR_DODGING_ARG_NAME, block_value);
...
{
    UHD_ASSERT_THROW(
        _tree->exists(get_arg_path(SPUR_DODGING_THRESHOLD_ARG_NAME) / "value"));
    auto block_value =
...
bool rhodium_radio_ctrl_impl::_get_highband_spur_reduction_enabled(
    ...
    uhd::rpc_client::sptr rpcc, const uhd::device_addr_t& block_args)
{
    ...
    UHD_LOG_TRACE(
        unique_id(), "Checking for existence of Rhodium DB in slot " << _radio_slot);
...
bool rhodium_radio_ctrl_impl::get_lo_lock_status(const direction_t dir) const
{
    return ((dir == RX_DIRECTION) or _tx_lo->get_lock_status())
           and ((dir == TX_DIRECTION) or _rx_lo->get_lock_status());
}
