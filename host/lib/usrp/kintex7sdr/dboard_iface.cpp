...
class kintex7sdr_dboard_iface : public dboard_iface {
public:
...
kintex7sdr_dboard_iface::~kintex7sdr_dboard_iface(void) {
    /* NOP */
}
 ...
 * Clocks
 **********************************************************************/
void kintex7sdr_dboard_iface::set_clock_rate(unit_t unit, double rate) {
    if (unit == UNIT_BOTH)
...
double kintex7sdr_dboard_iface::get_clock_rate(unit_t unit) {
    if (unit == UNIT_BOTH)
        throw uhd::runtime_error("UNIT_BOTH not supported.");
    return _clock_rates[unit]; // get from shadow
}

std::vector<double> kintex7sdr_dboard_iface::get_clock_rates(unit_t unit) {
    if (unit == UNIT_BOTH)
        throw uhd::runtime_error("UNIT_BOTH not supported.");
    switch (unit) {
        case UNIT_RX:
...
void kintex7sdr_dboard_iface::set_clock_enabled(unit_t unit, bool enb) {
    if (unit == UNIT_BOTH)
        throw uhd::runtime_error("UNIT_BOTH not supported.");
    switch (unit) {
        case UNIT_RX:
...
double kintex7sdr_dboard_iface::get_codec_rate(unit_t unit) {
    if (unit == UNIT_BOTH)
        throw uhd::runtime_error("UNIT_BOTH not supported.");
 ...
 * GPIO
 **********************************************************************/
void kintex7sdr_dboard_iface::set_pin_ctrl(unit_t unit, uint32_t value, uint32_t mask) {
    _gpio->set_pin_ctrl(unit, static_cast<uint16_t>(value), static_cast<uint16_t>(mask));
}

uint32_t kintex7sdr_dboard_iface::get_pin_ctrl(unit_t unit) {
    return static_cast<uint32_t>(_gpio->get_pin_ctrl(unit));
}

void kintex7sdr_dboard_iface::set_atr_reg(
        unit_t unit, atr_reg_t reg, uint32_t value, uint32_t mask) {
    _gpio->set_atr_reg(
            unit, reg, static_cast<uint16_t>(value), static_cast<uint16_t>(mask));
}

uint32_t kintex7sdr_dboard_iface::get_atr_reg(unit_t unit, atr_reg_t reg) {
    return static_cast<uint32_t>(_gpio->get_atr_reg(unit, reg));
}

void kintex7sdr_dboard_iface::set_gpio_ddr(unit_t unit, uint32_t value, uint32_t mask) {
    _gpio->set_gpio_ddr(unit, static_cast<uint16_t>(value), static_cast<uint16_t>(mask));
}

uint32_t kintex7sdr_dboard_iface::get_gpio_ddr(unit_t unit) {
    return static_cast<uint32_t>(_gpio->get_gpio_ddr(unit));
}

void kintex7sdr_dboard_iface::set_gpio_out(unit_t unit, uint32_t value, uint32_t mask) {
    _gpio->set_gpio_out(unit, static_cast<uint16_t>(value), static_cast<uint16_t>(mask));
}

uint32_t kintex7sdr_dboard_iface::get_gpio_out(unit_t unit) {
    return static_cast<uint32_t>(_gpio->get_gpio_out(unit));
}

uint32_t kintex7sdr_dboard_iface::read_gpio(unit_t unit) {
...
void kintex7sdr_dboard_iface::write_spi(
...
uint32_t kintex7sdr_dboard_iface::read_write_spi(
        unit_t unit, const spi_config_t &config, uint32_t data, size_t num_bits) {
    if (unit == UNIT_BOTH)
        throw uhd::runtime_error("UNIT_BOTH not supported.");
 ...
 * I2C
 **********************************************************************/
void kintex7sdr_dboard_iface::write_i2c(uint16_t addr, const byte_vector_t &bytes) {
    return _i2c_iface->write_i2c(addr, bytes);
}

byte_vector_t kintex7sdr_dboard_iface::read_i2c(uint16_t addr, size_t num_bytes) {
 ...
 * Aux DAX/ADC
 **********************************************************************/
void kintex7sdr_dboard_iface::_write_aux_dac(unit_t unit) {
...
void kintex7sdr_dboard_iface::write_aux_dac(unit_t unit, aux_dac_t which, double value) {
    if (unit == UNIT_BOTH)
        throw uhd::runtime_error("UNIT_BOTH not supported.");
    ...
    static const uhd::dict<unit_t, aux_dac_to_addr> unit_to_which_to_addr =
            map_list_of(UNIT_RX,
                        map_list_of(AUX_DAC_A, ad5623_regs_t::ADDR_DAC_B)(
                                AUX_DAC_B, ad5623_regs_t::ADDR_DAC_A)(AUX_DAC_C,
                                                                      ad5623_regs_t::ADDR_DAC_A)(AUX_DAC_D,
                                                                                                 ad5623_regs_t::ADDR_DAC_B))(
                    UNIT_TX,
                    map_list_of(AUX_DAC_A, ad5623_regs_t::ADDR_DAC_A)(
                            AUX_DAC_B, ad5623_regs_t::ADDR_DAC_B)(AUX_DAC_C,
                                                                  ad5623_regs_t::ADDR_DAC_B)(AUX_DAC_D,
                                                                                             ad5623_regs_t::ADDR_DAC_A));
    _dac_regs[unit].addr = unit_to_which_to_addr[unit][which];
    this->_write_aux_dac(unit);
}

double kintex7sdr_dboard_iface::read_aux_adc(unit_t unit, aux_adc_t which) {
...
uhd::time_spec_t kintex7sdr_dboard_iface::get_command_time() {
    return _wb_iface->get_time();
}

void kintex7sdr_dboard_iface::set_command_time(const uhd::time_spec_t &t) {
    _wb_iface->set_time(t);
}

void kintex7sdr_dboard_iface::set_fe_connection(
