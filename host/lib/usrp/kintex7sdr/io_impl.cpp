 ...
 **********************************************************************/
struct kintex7sdr_impl::io_impl
{
    io_impl(void)
 ...
 **********************************************************************/
void kintex7sdr_impl::io_impl::recv_pirate_loop(
 ...
 * Helper Functions
 **********************************************************************/
void kintex7sdr_impl::io_init(void)
{
    // create new io impl
    ...
    for (const std::string& mb : _mbc.keys()) {
        ...
        _io_impl->pirate_tasks.push_back(
            task::make(std::bind(&kintex7sdr_impl::io_impl::recv_pirate_loop,
                _io_impl.get(),
...
void kintex7sdr_impl::update_tick_rate(const double rate)
{
...
void kintex7sdr_impl::update_rx_samp_rate(
...
void kintex7sdr_impl::update_tx_samp_rate(
    const std::string& mb, const size_t dsp, const double rate)
{
    std::shared_ptr<sph::send_packet_streamer> my_streamer =
        std::dynamic_pointer_cast<sph::send_packet_streamer>(
...
void kintex7sdr_impl::update_rates(void)
{
    for (const std::string& mb : _mbc.keys()) {
...
void kintex7sdr_impl::update_rx_subdev_spec(
...
void kintex7sdr_impl::update_tx_subdev_spec(
    const std::string& which_mb, const subdev_spec_t& spec)
{
    fs_path root = "/mboards/" + which_mb + "/dboards";
 ...
 * Async Data
 **********************************************************************/
bool kintex7sdr_impl::recv_async_msg(async_metadata_t& async_metadata, double timeout)
{
    boost::this_thread::disable_interruption di; // disable because the wait can throw
 ...
 **********************************************************************/
void kintex7sdr_impl::program_stream_dest(
 ...
 * Receive streamer
 **********************************************************************/
rx_streamer::sptr kintex7sdr_impl::get_rx_stream(const uhd::stream_args_t& args_)
{
 ...
 * Transmit streamer
 **********************************************************************/
tx_streamer::sptr kintex7sdr_impl::get_tx_stream(const uhd::stream_args_t& args_)
{
    stream_args_t args = args_;
    ...
    for (size_t chan_i = 0; chan_i < args.channels.size(); chan_i++) {
        ...
        for (const std::string& mb : _mbc.keys()) {
            ...
            if (chan < num_chan_so_far) {
                ...
                my_streamer->set_xport_chan_get_buff(chan_i,
                    std::bind(&kintex7sdr_impl::io_impl::get_send_buff,
                        _io_impl.get(),
                        abs,
                        std::placeholders::_1));
