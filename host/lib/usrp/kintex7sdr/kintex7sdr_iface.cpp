...
class kintex7sdr_iface_impl : public kintex7sdr_iface {
public:
    /***********************************************************************
     * Structors
     **********************************************************************/
    kintex7sdr_iface_impl(udp_simple::sptr ctrl_transport)
            : _ctrl_transport(ctrl_transport), _ctrl_seq_num(0), _protocol_compat(0) // initialized below...
    {
    ...
    void lock_device(bool lock) override {
        if (lock) {
            ...
            _lock_task = task::make(std::bind(&kintex7sdr_iface_impl::lock_task, this));
        } else {
    ...
    usrp2_iface::rev_type get_rev(void) override {
        std::string hw = mb_eeprom["hardware"];
        if (hw.empty())
            return static_cast<usrp2_iface::rev_type>(USRP_NXXX);
        switch (boost::lexical_cast<uint16_t>(hw)) {
            case 0x0A21:
                return static_cast<usrp2_iface::rev_type>(USRP_N210_XK);
            case 0x0A20:
                return static_cast<usrp2_iface::rev_type>(USRP_N210_XA);
        }
