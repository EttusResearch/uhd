...
mpmd_xport_ctrl_dpdk_udp::mpmd_xport_ctrl_dpdk_udp(const uhd::device_addr_t& mb_args)
    : _mb_args(mb_args)
...
uhd::both_xports_t mpmd_xport_ctrl_dpdk_udp::make_transport(
    ...
    const uhd::device_addr_t& xport_args)
{
    ...
    if (xport_type == usrp::device3_impl::RX_DATA) {
        ...
        default_buff_args.recv_frame_size = std::min(
            xport_args.cast<size_t>("recv_frame_size", MPMD_10GE_DATA_FRAME_DEFAULT_SIZE),
            recv_mtu);
        ...
        default_buff_args.send_frame_size = std::min(
            xport_args.cast<size_t>("send_frame_size", MPMD_10GE_DATA_FRAME_DEFAULT_SIZE),
            send_mtu);
    }

    UHD_LOG_TRACE("BUFF",
        "num_recv_frames=" << default_buff_args.num_recv_frames
    ...
    xports.send_buff_size =
        (default_buff_args.send_frame_size - MPMD_UDP_RESERVED_FRAME_SIZE)
        * default_buff_args.num_send_frames;
