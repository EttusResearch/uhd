...
{
    auto xport_args = (xport_type == usrp::device3_impl::CTRL) ? uhd::device_addr_t()
                                                               : xport_args_;
    ...
    if (xport_args.has_key("send_frame_size")
        and xport_type == usrp::device3_impl::TX_DATA) {
    ...
    if (xport_args.has_key("recv_frame_size")
        and xport_type == usrp::device3_impl::RX_DATA) {
        size_t recv_frame_size = xport_args.cast<size_t>("recv_frame_size", recv_mtu);
    ...
    xports.recv_buff_size =
        default_buff_args.recv_frame_size * default_buff_args.num_recv_frames;
    xports.send_buff_size =
        default_buff_args.send_frame_size * default_buff_args.num_send_frames;
