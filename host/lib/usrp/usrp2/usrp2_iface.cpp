...
public:
	...
	usrp2_iface_impl(udp_simple::sptr ctrl_transport) :
			_ctrl_transport(ctrl_transport), _ctrl_seq_num(0), _protocol_compat(
					0) // initialized below...
	{
...
T get_reg(wb_addr_type addr, T data = 0) {
	...
	usrp2_ctrl_data_t in_data = this->ctrl_send_and_recv(out_data,
			MIN_PROTO_COMPAT_REG);
	UHD_ASSERT_THROW(
			ntohl(in_data.id) == USRP2_CTRL_ID_OMG_GOT_REGISTER_SO_BAD_DUDE);
 ...
 * SPI
 **********************************************************************/
uint32_t transact_spi(int which_slave, const spi_config_t &config,
		uint32_t data, size_t num_bits, bool readback) override
		{
	static const uhd::dict<spi_config_t::edge_t, int> spi_edge_to_otw =
			boost::assign::map_list_of(spi_config_t::EDGE_RISE,
					USRP2_CLK_EDGE_RISE)(spi_config_t::EDGE_FALL,
					USRP2_CLK_EDGE_FALL);
...
byte_vector_t read_i2c(uint16_t addr, size_t num_bytes) override
{
	...
	std::copy(in_data.data.i2c_args.data,
			in_data.data.i2c_args.data + num_bytes, result.begin());
	return result;
}
...
usrp2_ctrl_data_t ctrl_send_and_recv(const usrp2_ctrl_data_t &out_data,
	...
	for (size_t i = 0; i < CTRL_RECV_RETRIES; i++) {
		try {
			return ctrl_send_and_recv_internal(out_data, lo, hi,
					CTRL_RECV_TIMEOUT / CTRL_RECV_RETRIES);
		} catch (const timeout_error &e) {
			UHD_LOGGER_ERROR("USRP2") << "Control packet attempt " << i
					<< ", sequence number " << _ctrl_seq_num << ":\n"
					<< e.what();
		}
	}
	throw uhd::runtime_error(
			"link dead: timeout waiting for control packet ACK");
}

usrp2_ctrl_data_t ctrl_send_and_recv_internal(const usrp2_ctrl_data_t &out_data,
		uint32_t lo, uint32_t hi, const double timeout) {
	// fill in the seq number and send
	usrp2_ctrl_data_t out_copy = out_data;
	out_copy.proto_ver = htonl(_protocol_compat);
	out_copy.seq = htonl(++_ctrl_seq_num);
	_ctrl_transport->send(
			boost::asio::buffer(&out_copy, sizeof(usrp2_ctrl_data_t)));
			...
			throw uhd::runtime_error(
					str(
							boost::format(
									...
									% ((lo == hi) ?
											(boost::format("%d") % hi) :
											(boost::format("[%d to %d]") % lo
													% hi)) % compat
									% this->images_warn_help_message()));
		}
...
{
	uint32_t minor = this->get_reg<uint32_t, USRP2_REG_ACTION_FW_PEEK32>(
	U2_FW_REG_VER_MINOR);
...
std::string images_warn_help_message(void) override
{
	...
	try {
		...
		return str(
				boost::format(
						"Could not find %s and %s in your images path!\n%s")
						% fw_image % fpga_image
...
// escape char for multi-line cmd + newline + indent?
	...
	if (this->get_rev() == USRP2_REV3 or this->get_rev() == USRP2_REV4) {
		const std::string card_burner = uhd::find_utility(
				"usrp2_card_burner_gui.py");
		const std::string card_burner_cmd = str(
				boost::format(" %s\"%s\" %s--fpga=\"%s\" %s--fw=\"%s\"") % sudo
						% card_burner % ml % fpga_image_path % ml
						% fw_image_path);
		return str(
				boost::format("%s\n%s")
						% print_utility_error("uhd_images_downloader.py")
		...
		const std::string image_loader_path = (fs::path(uhd::get_pkg_path())
				/ "bin" / "uhd_image_loader").string();
		const std::string image_loader_cmd = str(
				boost::format(" \"%s\" %s--args=\"type=usrp2,addr=%s\"")
