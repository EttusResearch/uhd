...
device_addrs_t usrp2_find(const device_addr_t &hint_) {
	...
	if (not hint.has_key("addr")) {
		for (const if_addrs_t &if_addrs : get_if_addrs()) {
			...
			usrp2_addrs.insert(usrp2_addrs.begin(), new_usrp2_addrs.begin(),
					new_usrp2_addrs.end());
		}
		return usrp2_addrs;
	}
	...
	try {
		udp_transport = udp_simple::make_broadcast(hint["addr"],
				BOOST_STRINGIZE(USRP2_UDP_CTRL_PORT));
	} catch (const std::exception &e) {
		UHD_LOGGER_ERROR("USRP2") << "Cannot open UDP transport on "
				<< hint["addr"] << ": " << e.what();
	...
	try {
		udp_transport->send(
				boost::asio::buffer(&ctrl_data_out, sizeof(ctrl_data_out)));
	} catch (const std::exception &ex) {
		UHD_LOGGER_ERROR("USRP2") << "USRP2 Network discovery error "
				<< ex.what();
	} catch (...) {
	...
	while (true) {
		size_t len = udp_transport->recv(
				boost::asio::buffer(usrp2_ctrl_data_in_mem));
...
static mtu_result_t determine_mtu(const std::string &addr,
		const mtu_result_t &user_mtu) {
	udp_simple::sptr udp_sock = udp_simple::make_connected(addr,
			BOOST_STRINGIZE(USRP2_UDP_CTRL_PORT));
	...
	usrp2_ctrl_data_t *ctrl_data =
			reinterpret_cast<usrp2_ctrl_data_t*>(&buffer.front());
	...
	size_t min_recv_mtu = sizeof(usrp2_ctrl_data_t), max_recv_mtu =
			user_mtu.recv_mtu;
	size_t min_send_mtu = sizeof(usrp2_ctrl_data_t), max_send_mtu =
			user_mtu.send_mtu;
...
static zero_copy_if::sptr make_xport(const std::string &addr,
		const std::string &port, const device_addr_t &hints,
	...
	zero_copy_if::sptr xport = udp_zero_copy::make(addr, port,
			default_buff_args, ignored_params, filtered_hints);

	// Send a small data packet so the usrp2 knows the udp source port.
	// This setup must happen before further initialization occurs
	// or the async update packets will cause ICMP destination unreachable.
	static const uint32_t data[2] = { uhd::htonx(
			uint32_t(0 /* don't care seq num */)), uhd::htonx(
			uint32_t(USRP2_INVALID_VRT_HEADER)) };
...
usrp2_impl::usrp2_impl(const device_addr_t &_device_addr) :
	...
	user_mtu.recv_mtu = size_t(
			device_addr.cast<double>("recv_frame_size", udp_simple::mtu));
	user_mtu.send_mtu = size_t(
			device_addr.cast<double>("send_frame_size", udp_simple::mtu));
	...
	try {
		...
		UHD_LOGGER_INFO("USRP2") << "Current recv frame size: " << mtu.recv_mtu
				<< " bytes";
		UHD_LOGGER_INFO("USRP2") << "Current send frame size: " << mtu.send_mtu
				<< " bytes";
	...
	for (size_t mbi = 0; mbi < device_args.size(); mbi++) {
		...
		_mbc[mb].iface = usrp2_iface::make(
				udp_simple::make_connected(addr,
						BOOST_STRINGIZE(USRP2_UDP_CTRL_PORT)));
		_tree->create<std::string>(mb_path / "name").set(
				_mbc[mb].iface->get_cname());
		_tree->create<std::string>(mb_path / "fw_version").set(
				_mbc[mb].iface->get_fw_version_string());

		// check the fpga compatibility number
		const uint32_t fpga_compat_num = _mbc[mb].iface->peek32(
		U2_REG_COMPAT_NUM_RB);
		uint16_t fpga_major = fpga_compat_num >> 16, fpga_minor =
				fpga_compat_num & 0xffff;
		if (fpga_major == 0) { // old version scheme
			fpga_major = fpga_minor;
			fpga_minor = 0;
		}
		int expected_fpga_compat_num = std::min(USRP2_FPGA_COMPAT_NUM,
		...
		_mbc[mb].rx_dsp_xports.push_back(
				make_xport(addr, BOOST_STRINGIZE(USRP2_UDP_RX_DSP0_PORT),
						device_args_i, "recv"));
		...
		_mbc[mb].tx_dsp_xport = make_xport(addr,
				BOOST_STRINGIZE(USRP2_UDP_TX_DSP0_PORT), device_args_i, "send");
		UHD_LOGGER_TRACE("USRP2") << "Making transport for Control...";
		_mbc[mb].fifo_ctrl_xport = make_xport(addr,
				BOOST_STRINGIZE(USRP2_UDP_FIFO_CRTL_PORT), device_addr_t(), "");
		...
		_tree->create<double>(mb_path / "link_max_rate").set(
				USRP2_LINK_RATE_BPS);

		////////////////////////////////////////////////////////////////
		// setup the mboard eeprom
		////////////////////////////////////////////////////////////////
		_tree->create<mboard_eeprom_t>(mb_path / "eeprom").set(
				_mbc[mb].iface->mb_eeprom).add_coerced_subscriber(
				std::bind(&usrp2_impl::set_mb_eeprom, this, mb,
						std::placeholders::_1));
		...
		_mbc[mb].clock = usrp2_clock_ctrl::make(_mbc[mb].iface,
				_mbc[mb].spiface);
		_tree->create<double>(mb_path / "tick_rate").set_publisher(
				std::bind(&usrp2_clock_ctrl::get_master_clock_rate,
						_mbc[mb].clock)).add_coerced_subscriber(
				std::bind(&usrp2_impl::update_tick_rate, this,
						std::placeholders::_1));
		...
		case usrp2_iface::USRP_N210_R4: {
			...
			_tree->create<meta_range_t>(rx_codec_path / "gains/digital/range").set(
					meta_range_t(0, 6.0, 0.5));
			_tree->create<double>(rx_codec_path / "gains/digital/value").add_coerced_subscriber(
					std::bind(&usrp2_codec_ctrl::set_rx_digital_gain,
							_mbc[mb].codec, std::placeholders::_1)).set(0);
			_tree->create<meta_range_t>(rx_codec_path / "gains/fine/range").set(
					meta_range_t(0, 0.5, 0.05));
			_tree->create<double>(rx_codec_path / "gains/fine/value").add_coerced_subscriber(
		...
		////////////////////////////////////////////////////////////////
		_tree->create<sensor_value_t>(mb_path / "sensors/mimo_locked").set_publisher(
				std::bind(&usrp2_impl::get_mimo_locked, this, mb));
		_tree->create<sensor_value_t>(mb_path / "sensors/ref_locked").set_publisher(
				std::bind(&usrp2_impl::get_ref_locked, this, mb));

		////////////////////////////////////////////////////////////////
		// create frontend control objects
		////////////////////////////////////////////////////////////////
		_mbc[mb].rx_fe = rx_frontend_core_200::make(_mbc[mb].wbiface,
				U2_REG_SR_ADDR(SR_RX_FRONT));
		_mbc[mb].tx_fe = tx_frontend_core_200::make(_mbc[mb].wbiface,
				U2_REG_SR_ADDR(SR_TX_FRONT));

		_tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec").add_coerced_subscriber(
				std::bind(&usrp2_impl::update_rx_subdev_spec, this, mb,
						std::placeholders::_1));
		_tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec").add_coerced_subscriber(
				std::bind(&usrp2_impl::update_tx_subdev_spec, this, mb,
						std::placeholders::_1));

		const fs_path rx_fe_path = mb_path / "rx_frontends" / "A";
		const fs_path tx_fe_path = mb_path / "tx_frontends" / "A";

		_tree->create<std::complex<double>>(rx_fe_path / "dc_offset" / "value").set_coercer(
				std::bind(&rx_frontend_core_200::set_dc_offset, _mbc[mb].rx_fe,
						std::placeholders::_1)).set(
				std::complex<double>(0.0, 0.0));
		_tree->create<bool>(rx_fe_path / "dc_offset" / "enable").add_coerced_subscriber(
				std::bind(&rx_frontend_core_200::set_dc_offset_auto,
						_mbc[mb].rx_fe, std::placeholders::_1)).set(true);
		_tree->create<std::complex<double>>(rx_fe_path / "iq_balance" / "value").add_coerced_subscriber(
				std::bind(&rx_frontend_core_200::set_iq_balance, _mbc[mb].rx_fe,
						std::placeholders::_1)).set(
				std::complex<double>(0.0, 0.0));
		_tree->create<std::complex<double>>(tx_fe_path / "dc_offset" / "value").set_coercer(
				std::bind(&tx_frontend_core_200::set_dc_offset, _mbc[mb].tx_fe,
						std::placeholders::_1)).set(
				std::complex<double>(0.0, 0.0));
		_tree->create<std::complex<double>>(tx_fe_path / "iq_balance" / "value").add_coerced_subscriber(
				std::bind(&tx_frontend_core_200::set_iq_balance, _mbc[mb].tx_fe,
						std::placeholders::_1)).set(
				std::complex<double>(0.0, 0.0));
		...
		////////////////////////////////////////////////////////////////
		_mbc[mb].rx_dsps.push_back(
				rx_dsp_core_200::make(_mbc[mb].wbiface,
						U2_REG_SR_ADDR(SR_RX_DSP0), U2_REG_SR_ADDR(SR_RX_CTRL0),
						USRP2_RX_SID_BASE + 0, true));
		_mbc[mb].rx_dsps.push_back(
				rx_dsp_core_200::make(_mbc[mb].wbiface,
						U2_REG_SR_ADDR(SR_RX_DSP1), U2_REG_SR_ADDR(SR_RX_CTRL1),
						USRP2_RX_SID_BASE + 1, true));
		for (size_t dspno = 0; dspno < _mbc[mb].rx_dsps.size(); dspno++) {
			_mbc[mb].rx_dsps[dspno]->set_link_rate(USRP2_LINK_RATE_BPS);
			_tree->access<double>(mb_path / "tick_rate").add_coerced_subscriber(
					std::bind(&rx_dsp_core_200::set_tick_rate,
							_mbc[mb].rx_dsps[dspno], std::placeholders::_1));
			fs_path rx_dsp_path = mb_path / "rx_dsps" / dspno;
			_tree->create<meta_range_t>(rx_dsp_path / "rate/range").set_publisher(
					std::bind(&rx_dsp_core_200::get_host_rates,
							_mbc[mb].rx_dsps[dspno]));
			_tree->create<double>(rx_dsp_path / "rate/value").set(1e6) // some default
			.set_coercer(
					std::bind(&rx_dsp_core_200::set_host_rate,
							_mbc[mb].rx_dsps[dspno], std::placeholders::_1)).add_coerced_subscriber(
					std::bind(&usrp2_impl::update_rx_samp_rate, this, mb, dspno,
							std::placeholders::_1));
			_tree->create<double>(rx_dsp_path / "freq/value").set_coercer(
					std::bind(&rx_dsp_core_200::set_freq,
							_mbc[mb].rx_dsps[dspno], std::placeholders::_1));
			_tree->create<meta_range_t>(rx_dsp_path / "freq/range").set_publisher(
					std::bind(&rx_dsp_core_200::get_freq_range,
							_mbc[mb].rx_dsps[dspno]));
			_tree->create<stream_cmd_t>(rx_dsp_path / "stream_cmd").add_coerced_subscriber(
		...
		_mbc[mb].tx_dsp = tx_dsp_core_200::make(_mbc[mb].wbiface,
				U2_REG_SR_ADDR(SR_TX_DSP), U2_REG_SR_ADDR(SR_TX_CTRL),
				USRP2_TX_ASYNC_SID);
		...
		{ // This scope can be removed once we're able to do named captures
			...
			_tree->access<double>(mb_path / "tick_rate").add_coerced_subscriber(
					[this_tx_dsp](const double rate) {
						this_tx_dsp->set_tick_rate(rate);
					});
			_tree->create<meta_range_t>(mb_path / "tx_dsps/0/rate/range").set_publisher(
					[this_tx_dsp]() {
						return this_tx_dsp->get_host_rates();
					});
			_tree->create<double>(mb_path / "tx_dsps/0/rate/value").set(1e6) // some default
		...
		_tree->create<double>(mb_path / "tx_dsps/0/freq/value").set_coercer(
				[this, mb](const double rate) {
					return this->set_tx_dsp_freq(mb, rate);
				});
		_tree->create<meta_range_t>(mb_path / "tx_dsps/0/freq/range").set_publisher(
				[this, mb]() {
					return this->get_tx_dsp_freq_range(mb);
				});

		// setup dsp flow control
		const double ups_per_sec = device_args_i.cast<double>("ups_per_sec",
				20);
		const size_t send_frame_size =
				_mbc[mb].tx_dsp_xport->get_send_frame_size();
		const double ups_per_fifo = device_args_i.cast<double>("ups_per_fifo",
				8.0);
		_mbc[mb].tx_dsp->set_updates(
				(ups_per_sec > 0.0) ?
						size_t(100e6 /*approx tick rate*// ups_per_sec) : 0,
				(ups_per_fifo > 0.0) ?
						size_t(
								USRP2_SRAM_BYTES / ups_per_fifo
										/ send_frame_size) :
						0);
		...
		_mbc[mb].time64 = time64_core_200::make(_mbc[mb].wbiface,
				U2_REG_SR_ADDR(SR_TIME64), time64_rb_bases,
				mimo_clock_sync_delay_cycles);
		_tree->access<double>(mb_path / "tick_rate").add_coerced_subscriber(
				std::bind(&time64_core_200::set_tick_rate, _mbc[mb].time64,
						std::placeholders::_1));
		_tree->create<time_spec_t>(mb_path / "time/now").set_publisher(
				std::bind(&time64_core_200::get_time_now, _mbc[mb].time64)).add_coerced_subscriber(
				std::bind(&time64_core_200::set_time_now, _mbc[mb].time64,
						std::placeholders::_1));
		_tree->create<time_spec_t>(mb_path / "time/pps").set_publisher(
				std::bind(&time64_core_200::get_time_last_pps, _mbc[mb].time64)).add_coerced_subscriber(
				std::bind(&time64_core_200::set_time_next_pps, _mbc[mb].time64,
						std::placeholders::_1));
		// setup time source props
		_tree->create<std::string>(mb_path / "time_source/value").add_coerced_subscriber(
				std::bind(&time64_core_200::set_time_source, _mbc[mb].time64,
						std::placeholders::_1)).set("none");
		_tree->create<std::vector<std::string>>(mb_path / "time_source/options").set_publisher(
				std::bind(&time64_core_200::get_time_sources, _mbc[mb].time64));
		// setup reference source props
		_tree->create<std::string>(mb_path / "clock_source/value").add_coerced_subscriber(
				std::bind(&usrp2_impl::update_clock_source, this, mb,
						std::placeholders::_1)).set("internal");
		...
		_tree->create<std::vector<std::string>>(
				mb_path / "clock_source/options").set(clock_sources);
		...
		_tree->access<double>(mb_path / "tick_rate").add_coerced_subscriber(
				std::bind(&usrp2_fifo_ctrl::set_tick_rate, _mbc[mb].fifo_ctrl,
						std::placeholders::_1));

		////////////////////////////////////////////////////////////////////
		// create user-defined control objects
		////////////////////////////////////////////////////////////////////
		_mbc[mb].user = user_settings_core_200::make(_mbc[mb].wbiface,
				U2_REG_SR_ADDR(SR_USER_REGS));
		_tree->create<user_settings_core_200::user_reg_t>(mb_path / "user/regs").add_coerced_subscriber(
				std::bind(&user_settings_core_200::set_reg, _mbc[mb].user,
						std::placeholders::_1));
		...
		_tree->create<dboard_eeprom_t>(mb_path / "dboards/A/rx_eeprom").set(
				rx_db_eeprom).add_coerced_subscriber(
				std::bind(&usrp2_impl::set_db_eeprom, this, mb, "rx",
						std::placeholders::_1));
		_tree->create<dboard_eeprom_t>(mb_path / "dboards/A/tx_eeprom").set(
				tx_db_eeprom).add_coerced_subscriber(
				std::bind(&usrp2_impl::set_db_eeprom, this, mb, "tx",
						std::placeholders::_1));
		_tree->create<dboard_eeprom_t>(mb_path / "dboards/A/gdb_eeprom").set(
				gdb_eeprom).add_coerced_subscriber(
				std::bind(&usrp2_impl::set_db_eeprom, this, mb, "gdb",
						std::placeholders::_1));

		// create a new dboard interface and manager
		_mbc[mb].dboard_manager = dboard_manager::make(rx_db_eeprom,
				tx_db_eeprom, gdb_eeprom,
				make_usrp2_dboard_iface(_mbc[mb].wbiface,
						_mbc[mb].iface /*i2c*/, _mbc[mb].spiface,
						_mbc[mb].clock), _tree->subtree(mb_path / "dboards/A"));

		// bind frontend corrections to the dboard freq props
		const fs_path db_tx_fe_path = mb_path / "dboards" / "A"
				/ "tx_frontends";
		for (const std::string &name : _tree->list(db_tx_fe_path)) {
			_tree->access<double>(db_tx_fe_path / name / "freq" / "value").add_coerced_subscriber(
					std::bind(&usrp2_impl::set_tx_fe_corrections, this, mb,
							std::placeholders::_1));
		}
		const fs_path db_rx_fe_path = mb_path / "dboards" / "A"
				/ "rx_frontends";
		for (const std::string &name : _tree->list(db_rx_fe_path)) {
			_tree->access<double>(db_rx_fe_path / name / "freq" / "value").add_coerced_subscriber(
					std::bind(&usrp2_impl::set_rx_fe_corrections, this, mb,
							std::placeholders::_1));
		}
	}
...
// initialize io handling
	...
	for (const std::string &mb : _mbc.keys()) {
		...
		for (const std::string &name : _tree->list(root / "rx_dsps")) {
			_tree->access<double>(root / "rx_dsps" / name / "freq" / "value").set(
					0.0);
		}
		for (const std::string &name : _tree->list(root / "tx_dsps")) {
			_tree->access<double>(root / "tx_dsps" / name / "freq" / "value").set(
					0.0);
		}

		_tree->access<subdev_spec_t>(root / "rx_subdev_spec").set(
				subdev_spec_t(
						"A:"
								+ _tree->list(root / "dboards/A/rx_frontends").at(
										0)));
		_tree->access<subdev_spec_t>(root / "tx_subdev_spec").set(
				subdev_spec_t(
						"A:"
								+ _tree->list(root / "dboards/A/tx_frontends").at(
										0)));
		...
		if (_mbc[mb].gps and _mbc[mb].gps->gps_detected()) {
			...
			_tree->access<std::string>(root / "clock_source/value").set(
					"gpsdo");
		}
	}
}
...
usrp2_impl::~usrp2_impl(void) {
	UHD_SAFE_CALL(
			_pirate_task_exit = true; for (const std::string& mb : _mbc.keys()) { _mbc[mb].tx_dsp->set_updates(0, 0); })
}

void usrp2_impl::set_db_eeprom(const std::string &mb, const std::string &type,
...
sensor_value_t usrp2_impl::get_mimo_locked(const std::string &mb) {
	const bool lock = (_mbc[mb].wbiface->peek32(U2_REG_IRQ_RB) & (1 << 10))
			!= 0;
	return sensor_value_t("MIMO", lock, "locked", "unlocked");
}

sensor_value_t usrp2_impl::get_ref_locked(const std::string &mb) {
	const bool lock = (_mbc[mb].wbiface->peek32(U2_REG_IRQ_RB) & (1 << 11))
			!= 0;
	return sensor_value_t("Ref", lock, "locked", "unlocked");
}

void usrp2_impl::set_rx_fe_corrections(const std::string &mb,
		const double lo_freq) {
	if (not _ignore_cal_file) {
		apply_rx_fe_corrections(this->get_tree()->subtree("/mboards/" + mb),
				"A", lo_freq);
	}
}

void usrp2_impl::set_tx_fe_corrections(const std::string &mb,
		const double lo_freq) {
	if (not _ignore_cal_file) {
		apply_tx_fe_corrections(this->get_tree()->subtree("/mboards/" + mb),
				"A", lo_freq);
	}
}
...
double usrp2_impl::set_tx_dsp_freq(const std::string &mb, const double freq_) {
	...
	const double tick_rate = _tree->access<double>(
			"/mboards/" + mb + "/tick_rate").get();

	// calculate the DAC shift (multiples of rate)
	const int zone = std::max(
			std::min<int>(std::lround(new_freq / tick_rate), 2), -2);
	...
	UHD_LOG_TRACE("USRP2",
			"DSP Tuning: Requested " + std::to_string(freq_ / 1e6)
					+ " MHz, Using "
							"Nyquist zone " + std::to_string(zone)
					+ ", leftover DSP tuning: " + std::to_string(new_freq / 1e6)
					+ " MHz.");
...
meta_range_t usrp2_impl::get_tx_dsp_freq_range(const std::string &mb) {
	const double dac_rate = _tree->access<double>(
			"/mboards/" + mb + "/tick_rate").get()
...
void usrp2_impl::update_clock_source(const std::string &mb,
