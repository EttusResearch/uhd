...
void x300_impl::synchronize_dacs(const std::vector<radio_perifs_t*>& radios)
{
    ...
    for (size_t i = 0; i < radios.size(); i++) {
        radios[i]->ctrl->peek64(uhd::usrp::device3::radio::RB64_TIME_NOW); //Discard value. We are just timing the call
    }
    ...
    for (size_t i = 0; i < radios.size(); i++) {
        ...
        radios[i]->ctrl->poke32(uhd::usrp::device3::radio::sr_addr(uhd::usrp::device3::radio::DACSYNC), 0x1);    //Arm FRAMEP/N sync pulse
...
{
    boost::uint32_t adc_rb = iface->peek32(uhd::usrp::device3::radio::RB32_RX);
