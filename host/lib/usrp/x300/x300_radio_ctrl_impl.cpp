    ...
    for(const gpio_atr::gpio_attr_map_t::value_type attr:  gpio_atr::gpio_attr_map) {
        switch (attr.first){
            case usrp::gpio_atr::GPIO_SRC:
                ...
                if (_fp_attr == "FP0") {
                    _tree->create<std::vector<std::string>>(fs_path("gpio") / _fp_attr / attr.second)
                    ...
                    _tree->create<std::vector<std::string>>(fs_path("gpio") / _fp_attr / attr.second)
                        ...
                        .add_coerced_subscriber([this](const std::vector<std::string>&){
                            throw uhd::runtime_error("Secondary radio does not have front-panel GPIO source control.");
                        });
                }
                break;
...
void x300_radio_ctrl_impl::set_gpio_attr(
	...
	switch (attr_type) {
		...
		for (size_t i = 0; i < bit_mask.size(); i++) {
		    if (bit_mask[i] == 1) {
			attr_value[i] = usrp::gpio_atr::attr_value_map.at(attr_type).at(bit_value[i]);
		    }
		}
