...
constexpr double MIN_TICK_RATE = 120e6;
constexpr double MAX_TICK_RATE = 200e6;
        ...
        if (math::frequencies_are_equal(_system_ref_rate, 10e6)) {
            if (math::frequencies_are_equal(_master_clock_rate, 184.32e6)) {
                ...
                clocking_mode = m10M_120M_ZDEL;
                    ...
                    fp_compare_epsilon<double>(_master_clock_rate) >=MIN_TICK_RATE
                    && fp_compare_epsilon<double>(_master_clock_rate) <= MAX_TICK_RATE
                    ) {
