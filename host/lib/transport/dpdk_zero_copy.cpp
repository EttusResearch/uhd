...
void uhd_dpdk_ctx::init(const device_addr_t& user_args)
{
    ...
    if (!_init_done) {
        ...
        _mtu = dpdk_args.has_key("dpdk-mtu") ? dpdk_args.cast<size_t>("dpdk-mtu", 0)
                                             : DEFAULT_FRAME_SIZE;
        ...
        for (ssize_t i = 0; i < num_ports; i++) {
            ...
            for (const auto& arg : args) {
                if (arg.has_key("dpdk-mac") && arg["dpdk-mac"] == nics[i]["dpdk-mac"]) {
            ...
            if (nics[i].has_key("dpdk-ipv4") && nics[i].has_key("dpdk-io-cpu")) {
            ...
            UHD_LOG_TRACE("DPDK",
                "Found NIC(" << i << "):" << std::endl
                             << nics[i].to_pp_string());
        }
        uhd_dpdk_start(num_ports, io_cpu_map.data(), num_mbufs, mbuf_cache_size, _mtu);
        _init_done = true;
    }
}
...
int uhd_dpdk_ctx::get_port_id(
    std::array<uint8_t, 6> mac_addr, unsigned int& port_id) const
{
    UHD_ASSERT_THROW(is_init_done());
    int num_ports = uhd_dpdk_port_count();
...
public:
    dpdk_zero_copy_msb(struct uhd_dpdk_socket* sock,
        ...
        size_t frame_size)
        : _sock(sock), _buf(nullptr), _free_bufs(free_bufs), _frame_size(frame_size){};
    ...
    sptr get_new(double timeout)
    {
        ...
        return make(this, uhd_dpdk_buf_to_data(_sock, _buf), _frame_size);
    }
...
public:
        ...
        , _tx_empty_count(0)
    {
        ...
        UHD_ASSERT_THROW(
            xport_params.recv_frame_size < ctx.get_mtu() - UHD_DPDK_HEADERS_SIZE);
        UHD_ASSERT_THROW(
            xport_params.send_frame_size < ctx.get_mtu() - UHD_DPDK_HEADERS_SIZE);
...
dpdk_zero_copy::sptr dpdk_zero_copy::make(const struct uhd_dpdk_ctx& ctx,
    const unsigned int dpdk_port_id,
    const std::string& addr,
    const std::string& remote_port,
    const std::string& local_port,
