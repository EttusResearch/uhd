...
#
"""UHD Power Cal: USRP Calibration Utility Objects."""
...
def get_streamer(usrp, direction, chan):
    ...
    return usrp.get_rx_stream(stream_args) if direction == "rx" else usrp.get_tx_stream(stream_args)
...
def get_usrp_power(streamer, num_samps=NUM_SAMPS_PER_EST, chan=0):
    ...
    """
    recv_buffer = numpy.zeros((streamer.get_num_channels(), num_samps), dtype=numpy.complex64)
...
def subtract_power(p1_db, p2_db):
    """Return the power of p1 subtracted from p2.

    Both are given in logarithmic units (dB).
    """
...
class USRPCalibratorBase:
    ...
    Every USRP that can do power calibration needs to implement this.
    """
    ...
    def init(self, rate, tone_freq, amplitude):
        """Initialize device with finalized values.

        Note that __init__() needs to finish before the call site knows the
        rate, so we can't fold this into __init__().
        """
        if self._dir == "tx":
    ...
    def _get_frequencies(self, start_hint=None, stop_hint=None, step_hint=None):
        ...
        start_min = (
            self.min_freq
            or getattr(self._usrp, "get_{}_freq_range".format(self._dir))(self._chan).start()
        )
        ...
        stop_max = (
            self.max_freq
            or getattr(self._usrp, "get_{}_freq_range".format(self._dir))(self._chan).stop()
        )
    ...
    def init_frequencies(self, start_hint, stop_hint, step_hint):
        ...
        if self._dir == "tx":
            ...
            for freq in freqs:
                ...
                print(
                    "[TX] Noise floor: {:7.2f} MHz => {:+6.2f} dBm".format(
                        freq / 1e6, self._noise[freq]
                    )
                )
        else:  # Rx
            ...
            for freq in freqs:
                ...
                for gain in self._gains:
                    ...
                    print(
                        "[RX] Noise floor: {:7.2f} MHz / {} dB => {:+6.2f} dBFS".format(
                            freq / 1e6, gain, self._noise[freq][gain]
                        )
                    )
        return freqs
    ...
    def run_rx_cal(self, freq):
        ...
        self.log("Requesting input power: {:+.2f} dBm.".format(self.min_detectable_signal))
        usrp_input_power = self._meas_dev.set_power(self.min_detectable_signal)
        recvd_power = get_usrp_power(self._streamer)
        self.log(
            "Got input power: {:+.2f} dBm. Received power: {:.2f} dBFS. "
            "Requesting new input power: {:+.2f} dBm.".format(
                usrp_input_power, recvd_power, usrp_input_power + PWR_EST_IDEAL_LEVEL - recvd_power
            )
        )
        ...
        for _ in range(SIGPWR_LOCK_MAX_ITER):
            ...
            self.log("Requesting input power: {:+.2f} dBm.".format(usrp_input_power + power_delta))
            usrp_input_power = self._meas_dev.set_power(usrp_input_power + power_delta)
        if not siggen_locked:
            raise RuntimeError(
                "Unable to lock siggen within {} iterations! Last input power level: {:+6.2f} dBm.".format(
                    SIGPWR_LOCK_MAX_ITER, usrp_input_power
                )
            )
        self.log(
            "Locked signal generator in at input power level: {:+6.2f} dBm.".format(
                usrp_input_power
            )
        )
        ...
        for gain in self._gains:
            ...
            self.log(
                "Set gain to: {} dB. Got gain: {} dB.".format(
                    gain, self._usrp.get_rx_gain(self._chan)
                )
            )
            time.sleep(0.1)  # Settling time of the USRP, highly conservative
            ...
            if not PWR_EST_LLIM <= recvd_power <= PWR_EST_ULIM:
                power_delta = PWR_EST_IDEAL_LEVEL - recvd_power
                self.log(
                    "Adapting input power to: {:+.2f} dBm.".format(usrp_input_power + power_delta)
                )
                usrp_input_power = self._meas_dev.set_power(
    ...
    def store(self):
        ...
        database.write_cal_data(cal_key, cal_serial, cal_data.serialize())
        self.results = {}


class B200Calibrator(USRPCalibratorBase):
    """B2xx calibrator class."""
...
class E3XXCalibrator(USRPCalibratorBase):
    ...
    def stop(self, store=True):
        ...
        print(
            "===== Temperature range was {:.1f} to {:.1f} C".format(
                self._start_temp, self._stop_temp
            )
        )
...
def get_usrp_calibrator(usrp, meas_dev, direction, **kwargs):
    ...
    usrp_type = getattr(usrp, "get_usrp_{}_info".format(direction))().get("mboard_id")
    ...
    for _, obj in inspect.getmembers(sys.modules[__name__]):
        if (
            inspect.isclass(obj)
            and issubclass(obj, USRPCalibratorBase)
            and usrp_type in getattr(obj, "mboard_ids", "")
        ):
            return obj(usrp, meas_dev, direction, **kwargs)
    raise RuntimeError("No USRP calibrator object found for device type: {}".format(usrp_type))
