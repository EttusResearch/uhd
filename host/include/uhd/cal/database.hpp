...
/*! \brief Identify the source of calibration data, i.e., where was it stored.
 *
...
{
public:
    /*! \brief Return a calibration data set as a serialized string.
     *
     * Note: the \p source_type parameter can be used to specify where to read
     *       cal data from. However, this class only has access to RC and
     *       FILESYSTEM type cal data. ANY will pick FILESYSTEM data if both
     *       are available, and RC data if only RC data is available.
    ...
    /*! \brief Check if calibration data exists for a given source type.
     *
    ...
    /*! \brief Store calibration data to the local filesystem database.
     *
    ...
    /*! \brief Function type to return serialized cal data key and serial.
     *
    ...
    /*! \brief Register a lookup function for cal data.
     *
