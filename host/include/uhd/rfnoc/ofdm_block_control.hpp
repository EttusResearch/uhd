...
public:
    ...
    static const uint32_t REG_CP_INSERTION_LEN_ADDR;
    static const uint32_t REG_CP_INSERTION_CP_LEN_FIFO_LOAD_ADDR;
    static const uint32_t REG_CP_INSERTION_CP_LEN_FIFO_CLR_ADDR;
    static const uint32_t REG_CP_INSERTION_CP_LEN_FIFO_OCC_ADDR;
    static const uint32_t REG_CP_REMOVAL_LEN_ADDR;
    static const uint32_t REG_CP_REMOVAL_CP_LEN_FIFO_LOAD_ADDR;
    static const uint32_t REG_CP_REMOVAL_CP_LEN_FIFO_CLR_ADDR;
    static const uint32_t REG_CP_REMOVAL_CP_LEN_FIFO_OCC_ADDR;
    ...
    virtual void set_cp_insertion_length(const uint32_t cp_length) = 0;
    virtual uint32_t get_cp_insertion_length() = 0;
    virtual void set_cp_removal_length(const uint32_t cp_length) = 0;
    virtual uint32_t get_cp_removal_length() = 0;
    virtual void load_cp_insertion_fifo(const std::vector<uint32_t> cp_lengths) = 0;
    virtual void clear_cp_insertion_fifo() = 0;
    virtual uint32_t get_cp_insertion_fifo_occupancy() = 0;
    virtual void load_cp_removal_fifo(const std::vector<uint32_t> cp_lengths) = 0;
    virtual void clear_cp_removal_fifo() = 0;
    virtual uint32_t get_cp_removal_fifo_occupancy() = 0;
};
