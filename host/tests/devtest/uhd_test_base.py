...
class shell_application:
    ...
    def run(self, args=None):
        ...
        try:
            proc = Popen(
                cmd_line, stdout=PIPE, stderr=PIPE, close_fds=True, env=env, universal_newlines=True
            )
...
class uhd_example_test_case(uhd_test_case):
    ...
    def report_example_results(self, test_name, run_results):
        ...
        for key in sorted(run_results):
            ...
            self.report_result(test_name, key, run_results[key])
    ...
    def test_all(self):
        ...
        for test_name, test_args in test_params.items():
            ...
            if not "products" in test_args or (
                self.usrp_info["product"] in test_args.get("products", [])
            ):
                ...
                self.assertTrue(
                    ...
                    "Run results:\n{r}".format(t=test_name, r=yaml_dump_to_str(run_results)),
                )
...
class UHDPythonTestCase(uhd_test_case):
    ...
    def test_all(self):
        ...
        for test_name, test_args in test_params.items():
            time.sleep(15)  # Wait for X300 devices to reclaim them
            if "products" not in test_args or (
                self.usrp_info["product"] in test_args.get("products", [])
            ):
                run_results = self.run_test(test_name, test_args)
                passed = bool(run_results)
