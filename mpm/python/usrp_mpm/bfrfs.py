...
class BufferFS(object):
    ...
    def __init__(self, raw_data_buffer, max_size=None, alignment=None, log=None):
        ...
        self.log.trace(
            "EEPROM-FS header has {} valid entries.".format(len(header.get("entries", [])))
        )
    ...
    def _parse_header(self, buf):
        ...
        num_entries = num_entries_struct.unpack_from(buf, offset=default_hdr_struct.size)[0]
        ...
        for entry_idx in range(num_entries):
            ...
            entry_unpacked = entry_struct.unpack_from(buf, offset=entry_offset)
            entries.append(dict(list(zip(self.entry[toc_version][1], entry_unpacked))))
    ...
    def _trunc_buffer(self, buf, entries):
        ...
        """
        max_len = max([x["base"] + x["length"] for x in itervalues(entries)] + [0])
        return buf[:max_len]
    ...
    def get_blob(self, identifier, entries=None, buf=None):
        ...
        if identifier not in entries:
            raise RuntimeError("Requesting non-existent blob {}!".format(identifier))
    ...
    def set_blob(self, identifier, blob):
        ...
        if identifier_str not in self.entries and len(self.entries) >= self.max_entries:
        ...
        entry_base = self._find_base(entry_info, new_entries, alignment=alignment)
        self.log.trace("First attempt at finding a base yields: {}".format(entry_base))
        new_entries.pop(identifier, None)
        if entry_base is None:
            self.log.trace("First attempt to find a spot failed.")
            space_occupied = self._calc_space_occupied(new_entries, alignment=alignment)
            self.log.trace("Current blobs are occupying {} bytes.".format(space_occupied))
            if space_occupied + entry_info["length"] > self.max_size:
                raise RuntimeError("Not enough space to store blob!")
            new_entries, new_buffer = self._pack_entries(new_entries, self.buffer, alignment)
            entry_base = self._find_base(entry_info, new_entries, alignment=alignment)
            self.log.trace("2nd attempt at finding a base yields: {}".format(entry_base))
            if entry_base is None:
        ...
        buf_base = self.buffer[: self.entries_base] + self.pad * (
            self.entries_base - len(self.buffer[: self.entries_base])
        )
    ...
    def _find_base(self, new_entry, entries, alignment):
        ...
        self.log.trace("Trying to find a spot for blob `%s' of length %d", entry_id, entry_len)
        if entry_id in entries and (
            entry_len <= entries[entry_id]["length"]
            or entries[entry_id]["base"] == max((x["base"] for x in itervalues(entries)))
        ):
            self.log.trace(
        ...
        last_base = max(
            [x["base"] + x["length"] for x in itervalues(entries)] + [self.entries_base]
        )
        self.log.trace("New entry needs to go after address %d", last_base)
        new_base = align_addr(last_base, alignment)
        self.log.trace("New address is: %d (Alignment is: %d)", new_base, alignment)
        if new_base + entry_len < self.max_size:
            return new_base
        self.log.debug(
            "New base address %d and length %d would exceed EEPROM size", new_base, entry_len
        )
        return None
    ...
    def _update_toc(self, entries, toc_buf):
        ...
        new_toc = struct.Struct(self.default_header[0]).pack(self.magic, 0) + struct.Struct(
            "!I"
        ).pack(len(entries))
        entry_struct = struct.Struct(self.entry[toc_version][0])
