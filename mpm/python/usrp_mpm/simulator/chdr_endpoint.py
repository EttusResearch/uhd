...
from .rfnoc_graph import NodeType, RFNoCGraph, StreamEndpointNode, XbarNode, XportNode
...
class ChdrEndpoint:
    ...
    def __init__(self, log, config):
        ...
        self.graph = RFNoCGraph(
            self.get_default_nodes(),
            self.log,
            0,
            self.send_wrapper,
            CHDR_W,
            config.hardware.rfnoc_device_type,
        )
        ...
        while True:
            ...
            for sock in ready_list:
                if sock is main_sock:
                    ...
                    self.log.trace("Received {} bytes of data from {}".format(n_bytes, sender))
                    try:
                        ...
                        response = self.graph.handle_packet(
                            packet, entry_xport, sender, sender, n_bytes
                        )

                        if response is not None:
                            data = response.serialize()
                            self.log.trace(
                                "Returning Packet: {}".format(packet.to_string_with_payload())
                            )
                            main_sock.sendto(bytes(data), sender)
                    except BaseException as ex:
                        self.log.warning("Unable to decode packet: {}".format(ex))
                        raise ex
                else:
