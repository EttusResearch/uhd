...
from usrp_mpm.periph_manager.x4xx_clock_lookup import RFDC_PLL_CONFIGS, MCR_LMK_VCO
...
class X440ClockPolicy(X4xxClockPolicy):
    ...
    def _match_bypass_freqs(self, mcrs):
        ...
        if (mcrs[0] == mcrs[1] or
            ...
            for dec in X4xxRfdcCtrl.RFDC_RESAMPLER:
                if (lmk_od % dec == 0 and
                    X4xxRfdcCtrl.CONV_RATE_MIN <= lmk_vco/(lmk_od/dec)
    ...
    def _match_mcrs(self, desired_mcr, desired_conv_rate=0):
        ...
        if div not in X4xxRfdcCtrl.RFDC_RESAMPLER or not \
            X4xxRfdcCtrl.CONV_RATE_MIN <= desired_conv_rate <= X4xxRfdcCtrl.CONV_RATE_MAX or \
            desired_mcr not in MCR_LMK_VCO:
    ...
    def _get_min_mcr(self):
        ...
        """
        return X4xxRfdcCtrl.CONV_RATE_MIN \
    ...
    def get_config(self, ref_clock_freq, master_clock_rates):
        ...
        assert X4xxRfdcCtrl.MMCM_FB_MIN <= mmcm_fb <= X4xxRfdcCtrl.MMCM_FB_MAX, \
            "Invalid MMCM Feedback Divider."
        r0_clk = int(mmcm_vco_rate / rfdc_rate[0])
        assert X4xxRfdcCtrl.MMCM_OD_MIN <= r0_clk <= X4xxRfdcCtrl.MMCM_OD_MAX, \
            "Invalid MMCM output divider for radio 0 clock."
        r1_clk = int(mmcm_vco_rate / rfdc_rate[1])
        assert X4xxRfdcCtrl.MMCM_OD_MIN <= r1_clk <= X4xxRfdcCtrl.MMCM_OD_MAX, \
            "Invalid MMCM output divider for radio 1 clock."
        r0_clk2x = int(r0_clk/2)
        assert X4xxRfdcCtrl.MMCM_OD_MIN <= r0_clk2x <= X4xxRfdcCtrl.MMCM_OD_MAX, \
            "Invalid MMCM output divider for radio 0 clock 2x."
        r1_clk2x = int(r1_clk/2)
        assert X4xxRfdcCtrl.MMCM_OD_MIN <= r1_clk2x <= X4xxRfdcCtrl.MMCM_OD_MAX, \
        ...
        clk_config = {
            ...
            'mmcm_output_div_map': {'r0_clk': r0_clk, 'r1_clk': r1_clk, 'r0_clk_2x': r0_clk2x,
                                    'r1_clk_2x': r1_clk2x, 'prc': prc_out_div},
