...
from usrp_mpm.periph_manager.x4xx_clock_lookup import LMK_VCO_RATES, RFDC_PLL_CONFIGS
...
class X440ClockPolicy(X4xxClockPolicy):
    ...
    clocking policy of the X410 it uses the RFDC PLL and MMCM and
    ...
    def validate_ref_clock_freq(self, ref_clock_freq, master_clock_rates):
        ...
        sysref_config = next(sysref_setting
                          for sysref_setting in LMK04832X4xx.SYSREF_CONFIG[100e6
                          if config.spll_config.vcxo_freq==Spll1Vco.VCO100MHz else 122.88e6]
                          if sysref_setting['SYSREF_FREQ'] in self._find_sysref_matches(
    ...
    def _read_converter_rates(self):
        """
        Gets the converter rate from the the argument string if it is given there, None otherwise.
        """
    ...
    def _match_bypass_freqs(self, mcrs):
        ...
        # Check if this rate can be generated by the LMK and if it can be achieved by only dividing
        # through the allowed decimation factors:
        lmk_direct_rates = [x for x in LMK_VCO_RATES if not x % required_rate]
        lmk_vco = None
        lmk_od = None
        if len(lmk_direct_rates) == 0:
            return False, lmk_vco, lmk_od

        if (mcrs[0] == mcrs[1] or
            all(int(required_rate / mcr) in X4xxRfdcCtrl.RFDC_RESAMPLER for mcr in mcrs)):
            lmk_vco = int(min(lmk_direct_rates))
            lmk_od = int(lmk_vco / required_rate)
            # Let the RFDC decimation do most work:
            for dec in X4xxRfdcCtrl.RFDC_RESAMPLER:
                if (lmk_od % dec == 0 and
                    X4xxRfdcCtrl.ADC_CONV_RATE_MIN <= lmk_vco/(lmk_od/dec)
                    <= X4xxRfdcCtrl.ADC_CONV_RATE_MAX):
    ...
    def _reduce_rfdc_frequency_list(self, resampling_factor):
        """
        Filters the RFDC frequency list to only contain frequencies that can be supported with
        a valid SYSREF frequency.
        """
        # Filter the RFDC frequencies to only contain those that can be achieved with a valid
        # SYSREF frequency:
        return [rate for rate in RFDC_PLL_CONFIGS.keys()
              if len(self._find_sysref_matches(rate / resampling_factor)) > 0]

    def _find_sysref_matches(self, mcr):
        """
        Finds all SYSREF frequencies that are compatible with the given converter rate and resampling
        factor.
        """
        # Find all SYSREF frequencies that are compatible with the given converter rate and
        # resampling factor:
        return [sysref_rate for sysref_rate in self._valid_sysref_freqs
    ...
    def _match_mcrs(self, desired_mcr, desired_conv_rate=0):
        ...
        if div not in X4xxRfdcCtrl.RFDC_RESAMPLER or not \
            X4xxRfdcCtrl.ADC_CONV_RATE_MIN <= desired_conv_rate <= X4xxRfdcCtrl.ADC_CONV_RATE_MAX:
            desired_conv_rate, div = next((desired_mcr * k, k) for k in
                                        sorted(X4xxRfdcCtrl.RFDC_RESAMPLER, reverse=True)
                                        if desired_mcr * k <= X4xxRfdcCtrl.ADC_CONV_RATE_MAX)
        # Coerce. For excluding too many difficult-to-achieve MMCM VCO rates we restrict the
        # RFDC rate to non-fractional values.
        coerced_conv_rate = min(
            (rate for rate in self._reduce_rfdc_frequency_list(div)
             if rate / div % (self._spc * self._extra_resampling) == 0),
            key=lambda x: abs(x-desired_conv_rate))
        return coerced_conv_rate / div, coerced_conv_rate

    def _get_common_spll_out_freqs(self, conv_rates):
        """
        Calculates possible common LMK output frequencies to achieve the two converter rates
        """
        return list(set.intersection(*[set(v) for k,v in RFDC_PLL_CONFIGS.items()
                                       if k in conv_rates]))
    ...
    def _get_min_mcr(self):
        ...
        """
        return X4xxRfdcCtrl.ADC_CONV_RATE_MIN \
                / max(X4xxRfdcCtrl.RFDC_RESAMPLER) \
                / self._extra_resampling

    def _get_mmcm_rates(self, rfdc_rate, lmk_vco):
        """
        Find MMCM config
        """
        ...
        for mmcm_vco_rate in range(min_mmcm_vco_rate, int(X4xxRfdcCtrl.MMCM_VCO_MAX+1),
                                   min_common_rfdc_rate):
            ...
            if X4xxRfdcCtrl.MMCM_FB_MIN <= mmcm_fb <= X4xxRfdcCtrl.MMCM_FB_MAX:
                # Divide this mmcm_input until it fits into the MMCM input
                for div in range(mmcm_fb, X4xxRfdcCtrl.MMCM_FB_MAX+1, mmcm_fb):
                    mmcm_input = mmcm_vco_rate / div
                    if (X4xxRfdcCtrl.MMCM_INPUT_MIN <= mmcm_input <= X4xxRfdcCtrl.MMCM_INPUT_MAX and
                        not (mmcm_input) % 1 and
                        not (lmk_vco / mmcm_input) % 1):
                        break
                    mmcm_input = False
            if mmcm_input:
                break
        return mmcm_input, mmcm_vco_rate
    ...
    def coerce_mcr(self, master_clock_rates):
        ...
        common_out = self._get_common_spll_out_freqs(conv_rates)
        ...
        if self.conv_rates is not None and self.conv_rates != conv_rates:
            self.log.warning(f"Unable to use desired converter rate(s), using "
    ...
    def get_config(self, ref_clock_freq, master_clock_rates):
        """
        Returns a valid configuration based on the master clock rate. It uses the
        configuration where the RFDC_CLOCK/SPC is the closest to the MCR
        ...
        bypass_conv_rate = 0 if lmk_vco is None else lmk_vco / lmk_od
        ...
        if bypass and bypass_conv_rate in conv_rates:
            ...
            lmk_possible_vco_rates = list(filter(lambda x: not x % output_freq, LMK_VCO_RATES))
            lmk_vco = int(max(lmk_possible_vco_rates))
        ...
        rfdc_rate = list(map(lambda x: int(x / (self._spc * self._extra_resampling)), mcrs))
        mmcm_input, mmcm_vco_rate = self._get_mmcm_rates(rfdc_rate, lmk_vco)
        prc_rate = mmcm_input
        ...
        assert sysref_config['SYSREF_FREQ'] < 10e6
        assert all(
        ...
        rfdc_cfg0 = {
            'conv_rate': conv_rates[0],
            'resampling': int(conv_rates[0] / mcrs[0])
            }
        rfdc_cfg1 = {
            'conv_rate': conv_rates[1],
            'resampling': int(conv_rates[1] / mcrs[1])
            }
        clk_config = {
            'mmcm_use_defaults': False,
            'mmcm_feedback_divider': mmcm_fb,
            'mmcm_input_divider': 1,
            'mmcm_output_div_map': {'r0_clk': r0_clk, 'r1_clk': r1_clk, 'r0_clk_2x': r0_clk,
                                    'r1_clk_2x': r1_clk, 'prc': prc_out_div},
