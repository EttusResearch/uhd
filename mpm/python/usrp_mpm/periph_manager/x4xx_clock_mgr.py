...
from usrp_mpm.periph_manager.x4xx_clock_types import BrcSource, RpllRefSel
...
class X4xxClockManager:
    ...
    def __init__(self, args, clk_policy, clk_aux_board, cpld_control, log):
        ...
        # Parse args
        self._time_source = args.get("time_source", self._safe_sync_source["time_source"])
        self._clock_source = args.get("clock_source", self._safe_sync_source["clock_source"])
    ...
    def _init_available_srcs(self):
        ...
        if self._clocking_auxbrd:
            self._avail_clk_sources.extend([self.CLOCK_SOURCE_INTERNAL, self.CLOCK_SOURCE_EXTERNAL])
    ...
    def _init_ref_clock_and_time(self):
        ...
        self._config_rpll(
            self.X400_DEFAULT_MGT_CLOCK_RATE, self.X400_DEFAULT_INT_CLOCK_FREQ, RpllRefSel.SECONDARY
        )
        clk_config = self.clk_policy.get_config(self.get_ref_clock_freq(), self._master_clock_rates)
    ...
    def finalize_init(self, args, mboard_regs_control, rfdc):
        ...
        initial_mcr = float(args.get("master_clock_rate", self.clk_policy.get_default_mcr()))
        try:
    ...
    def init(self, args):
        ...
        if desired_master_clock_rates:
            desired_master_clock_rates = parse_multi_device_arg(
                args["master_clock_rate"], conv=float
            )
            if len(desired_master_clock_rates) == 1 and len(self._master_clock_rates) > 1:
                desired_master_clock_rates = desired_master_clock_rates * len(
                    self._master_clock_rates
                )
        ...
        if desired_converter_rates and not desired_master_clock_rates:
            self.log.warning(
                "Passing `converter_rate` argument without obligatory "
    ...
    def _set_sync_source(self, clock_source, time_source, force=False):
        ...
        self.log.debug(
            f"Setting sync source to time_source={time_source}, " f"clock_source={clock_source}"
        )
    ...
    def _config_rpll(
        self, usr_clk_rate: float, internal_brc_rate: float, internal_brc_source: RpllRefSel
    ):
        """
        ...
        if internal_brc_source == RpllRefSel.PRIMARY and self._rpll_priref_rate is None:
            err = f"Invalid internal BRC source '{internal_brc_source}' was selected."
            self.log.error(err)
            raise RuntimeError(err)
        ref_rate = (
            self._rpll_priref_rate
            if internal_brc_source == RpllRefSel.PRIMARY
            else self.X400_RPLL_SECREF_RATE
        )
        self.clk_ctrl.config_rpll(internal_brc_source, ref_rate, internal_brc_rate, usr_clk_rate)
    ...
    def _set_ref_clock_freq(self, freq, master_clock_rates, update_clocks=True):
        ...
        if update_clocks and self.get_clock_source() == self.CLOCK_SOURCE_EXTERNAL:
            self.set_sync_source(
                {"clock_source": self._clock_source, "time_source": self._time_source}
            )
    ...
    def _set_brc_source(self, clock_source, time_source):
        ...
        if clock_source == self.CLOCK_SOURCE_MBOARD:
            ...
            if self._clocking_auxbrd is None:
                self.log.error(
                    f"Invalid BRC selection {clock_source}. No clocking aux " "board was found."
                )
    ...
    def set_master_clock_rate(self, master_clock_rates):
        ...
        clk_settings = self.clk_policy.get_config(self.get_ref_clock_freq(), master_clock_rates)
        self.log.debug(f"Clock Config: {clk_settings}")
        self.log.info(
            f"Using Clock Configuration:\n"
        ...
        for tk_idx, mcr in enumerate(master_clock_rates):
            self.clk_ctrl.configure_pps_forwarding(
                tk_idx, True, self.clk_policy.get_radio_clock_rate(mcr)
            )
    ...
    def get_converter_rate(self, db_idx):
        ...
        """
        if not self.rfdc:
            self.log.info("Querying converter rate from SPLL, RFDC is not yet enabled!")
    ...
    def set_clock_source(self, *args):
        ...
        if (clock_source, time_source) not in self.valid_sync_sources:
            ...
            if clock_source in (self.CLOCK_SOURCE_MBOARD, self.CLOCK_SOURCE_INTERNAL):
                time_source = self.TIME_SOURCE_INTERNAL
        ...
        self.set_sync_source({"clock_source": clock_source, "time_source": time_source})
    ...
    def set_clock_source_out(self, enable):
        ...
        if self.get_time_source() == self.TIME_SOURCE_EXTERNAL:
            raise RuntimeError("Cannot export clock when using external time reference!")
    ...
    def set_sync_source(self, args):
        ...
        if isinstance(master_clock_rates, str):
            master_clock_rates = parse_multi_device_arg(master_clock_rates, conv=float)
        ...
        if clock_source == self.CLOCK_SOURCE_EXTERNAL:
            self.clk_policy.validate_ref_clock_freq(self.get_ref_clock_freq(), master_clock_rates)
        ...
        # exported either.
        if (
            clock_source in (self.CLOCK_SOURCE_EXTERNAL, self.CLOCK_SOURCE_MBOARD)
        ...
        try:
            ...
            if (
                self.clk_policy.should_reboot_on_reconfiguration()
                ...
                if initializing:
                    ...
                    raise RuntimeError(
                        "Changing clocking configuration after device "
            ...
            if skip_mpm_reboot:
                self.log.warning(
                    "Overriding recommended MPM reboot during clocking configuration. "
            ...
            interm_clk_settings = self.clk_policy.get_intermediate_clk_settings(
                self.get_ref_clock_freq(), self._master_clock_rates, master_clock_rates
            )
            if interm_clk_settings:
                self.log.debug("Applying intermediate clock settings.")
                self._configure_clock_chain(
                    interm_clk_settings, self.get_time_source(), self.get_ref_clock_freq()
                )
            ...
            err = (
                f"Setting clock_source={clock_source},time_source={time_source} "
    ...
    def get_clocks(self):
        ...
        return [
            ...
            {"name": "4", "freq": str(self.get_master_clock_rate(0)), "mutable": "true"},
            # We don't need an alias for clock 4, because it is not referenced
            # in the block registry.
            {"name": "5", "freq": str(self.get_master_clock_rate(1)), "mutable": "true"},
    ...
    def synchronize(self, sync_args, finalize):
        ...
        def determine_sync_settings():
            ...
            if rates[0] != rates[1] or (
                ...
                and self._master_clock_rates[0] != self._master_clock_rates[1]
            ):
                self.log.info(
                    "Multiple master clock rates detected: Skipping Multi-Tile "
            ...
            for db_key, adc_lat_key, dac_lat_key in db_keys:
                adc_latencies, dac_latencies = self.rfdc.determine_tile_latencies(db_key)
                self.log.debug(
        ...
        def finalize_sync_settings(sync_args):
            ...
            if "adc_latency" in sync_args and "dac_latency" in sync_args:
                return self.rfdc.set_tile_latencies(
                    "all", int(sync_args["adc_latency"]), int(sync_args["dac_latency"])
                )
            if len(sync_args) == 0:
    ...
    def config_rpll_to_nsync(self):
        ...
        self._config_rpll(
            self.X400_DEFAULT_MGT_CLOCK_RATE, self.X400_DEFAULT_INT_CLOCK_FREQ, RpllRefSel.PRIMARY
        )
