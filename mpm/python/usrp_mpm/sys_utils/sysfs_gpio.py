...
def get_all_gpio_devs(parent_dev=None):
    ...
    try:
        ...
        gpios = [
            device.sys_name
            for device in context.list_devices(subsystem="gpio").match_parent(parent_dev)
...
class SysFSGPIO(object):
    ...
    def __init__(self, identifiers, use_mask, ddr, init_value=0, parent_dev=None):
        ...
        self._out_value = 0
        self.log.trace("Generating SysFSGPIO object for identifiers `{}'...".format(identifiers))
        self._gpio_dev, self._map_info = find_gpio_device(identifiers, parent_dev, self.log)
        if self._gpio_dev is None:
            error_msg = "Could not find GPIO device with identifiers `{}'.".format(identifiers)
        ...
        self.init(
            self._map_info["ngpio"], self._base_gpio, self._use_mask, self._ddr, self._init_value
        )
...
class GPIOBank:
    ...
    def __init__(self, uio_identifiers, offset, usemask, ddr):
        ...
        self._gpios = SysFSGPIO(
            uio_identifiers, self._usemask << self._offset, self._ddr << self._offset
        )
