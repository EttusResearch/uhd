...
class Neon(AD936xDboard, DboardManagerBase):
    ...
    def __init__(self, slot_idx, **kwargs):
        ...
        self.log.trace("Initializing Neon daughterboard, slot index %d", self.slot_idx)
    ...
    def _init_periphs(self):
        ...
        adf4002_spi = lib.spi.make_spidev(
            str(self._spi_nodes["adf4002"]), 1000000, 0  # Speed (Hz)  # SPI mode
        )
        self.log.trace("Initializing ADF4002.")
        from usrp_mpm.periph_manager.e320 import E320_DEFAULT_INT_CLOCK_FREQ

        self.adf4002 = ADF400x(adf4002_spi, freq=E320_DEFAULT_INT_CLOCK_FREQ, parent_log=self.log)
    ...
    def _get_user_eeprom_info(self, rev):
        ...
        while rev_for_lookup not in self.user_eeprom:
            if rev_for_lookup < 0:
                raise RuntimeError(f"Could not find a user EEPROM map for revision {rev}!")
            rev_for_lookup -= 1
        assert rev_for_lookup in self.user_eeprom, "Invalid EEPROM lookup rev!"
    ...
    def _init_user_eeprom(self, eeprom_info):
        ...
        self.log.trace("Found the following EEPROM paths: `{}'".format(eeprom_paths))
    ...
    def get_user_eeprom_data(self):
        ...
        """
        return {
            blob_id: self.eeprom_fs.get_blob(blob_id) for blob_id in self.eeprom_fs.entries.keys()
        }
    ...
    def set_user_eeprom_data(self, eeprom_data):
        ...
        def _write_to_eeprom_task(path, offset, data, log):
            ...
            with open(path, "r+b") as eeprom_file:
                ...
                log.trace("Writing a total of {} bytes.".format(len(self.eeprom_fs.buffer)))
        ...
        writer_task = threading.Thread(
            ...
            args=(self.eeprom_path, eeprom_offset, self.eeprom_fs.buffer, self.log),
            name=thread_id,
        )
        writer_task.start()
