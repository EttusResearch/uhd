...
#
# Description:
#   Timing constraints exclusive to X420. These should be used
...
set_input_delay  -clock pll_ref_clk -min $db_gpio_fpga_min_in  $db_gpio_ports
set_input_delay  -clock pll_ref_clk -max $db_gpio_fpga_max_in  $db_gpio_ports
set_output_delay -clock pll_ref_clk -min $db_gpio_fpga_min_out $db_gpio_ports
set_output_delay -clock pll_ref_clk -max $db_gpio_fpga_max_out $db_gpio_ports

# Special handling of sync injection ports (overwrites previous constraints).
# Timing values guarantee an IOB placement of the output registers.
set db_gpio_sync_ports [get_ports {DB0_GPIO[19] DB1_GPIO[10]}]
set_output_delay -clock ref_clk -max 0.0 $db_gpio_sync_ports
set_max_delay -from [get_cells -hierarchical "clk_out_iob_reg*" ] -datapath_only 3.1
...
# Search the nets beginning from the MMCM outputs to find the clock buffers outputs.
...
# code clocked in this domain will be reset after this clock is enabled. This
