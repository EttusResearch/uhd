...
//
//   This module incurs one clk1x cycle of delay plus one additional clk2x
//   cycle of delay on the data and valid signals from input on the 1x domain
//   to output on the 2x domain.
//
...
module adc_gearbox_8x4 (
  ...
  // crossing.
  reg toggle_1x, toggle_2x = 1'b0, toggle_2x_dly = 1'b0;
  reg valid_2x = 1'b0, valid_dly_2x = 1'b0, valid_dly2_2x = 1'b0;
  reg [127:0] data_out_2x = 128'b0, adc_q_data_in_2x = 128'b0, adc_i_data_in_2x = 128'b0;
  // Pipeline stage registers: selected 64-bit I/Q values for stage 2.
  reg [63:0] adc_i_sel_2x = 64'b0, adc_q_sel_2x = 64'b0;
  ...
  // 2x domain by one cycle and compare it to the non-delayed version. When they
  // differ, push data_in[63:0] into the clk2x register. When they match, push
  // [127:64] into the clk2x register. The datasheet is unclear on the exact
  // implementation.
  //
  ...
  // registers.
  // Stage 1: Transfer data and select the appropriate 64-bit half based on
  // the toggle comparison.
  always @(posedge clk2x) begin
    ...
    if (toggle_2x != toggle_2x_dly) begin
      adc_i_sel_2x <= adc_i_data_in_2x[63:0];
      adc_q_sel_2x <= adc_q_data_in_2x[63:0];
    end else begin
      adc_i_sel_2x <= adc_i_data_in_2x[127:64];
      adc_q_sel_2x <= adc_q_data_in_2x[127:64];
    end
    valid_2x     <= valid_in_1x && enable_1x;
    valid_dly_2x <= valid_2x;
  end

  // Stage 2: Interleave I/Q samples and apply swap.
  // Place Q in the MSBs, I in the LSBs by default, unless swapped = 1.
  always @(posedge clk2x) begin
    data_out_2x <= 128'b0;
    if (valid_dly_2x) begin
      if (swap_iq_2x) begin
        data_out_2x <= {adc_i_sel_2x[63:48], adc_q_sel_2x[63:48],
                        adc_i_sel_2x[47:32], adc_q_sel_2x[47:32],
                        adc_i_sel_2x[31:16], adc_q_sel_2x[31:16],
                        adc_i_sel_2x[15: 0], adc_q_sel_2x[15: 0]};
      end else begin
        data_out_2x <= {adc_q_sel_2x[63:48], adc_i_sel_2x[63:48],
                        adc_q_sel_2x[47:32], adc_i_sel_2x[47:32],
                        adc_q_sel_2x[31:16], adc_i_sel_2x[31:16],
                        adc_q_sel_2x[15: 0], adc_i_sel_2x[15: 0]};
      end
    end
    valid_dly2_2x <= valid_dly_2x;
  end

  assign adc_out_2x   = data_out_2x;
  assign valid_out_2x = valid_dly2_2x;
