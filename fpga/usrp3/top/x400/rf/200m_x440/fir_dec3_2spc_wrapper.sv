...
//
// Module: fir_dec3_2spc_wrapper
//
// Description:
//
//   Wrapper for the Xilinx FIR Compiler IP (fir_dec3_2spc) configured as a
//   decimate-by-3 filter with 77 symmetric taps, 18-bit coefficients, 2
//   parallel data paths (I/Q), 2 samples per clock input, and 16-bit
//   signed input data. The output is full-precision 34-bit.
//
//  The FIR filter coefficients are reused from
//  fpga/usrp3/top/x400/ip/adc_400m_bd/adc_400m_bd.tcl
//
//   IP Configuration Summary (from fir_dec3_2spc.xci):
...
//
//   s_axis_data_tdata packing (2 data paths, 2 SPC, 16 bits each):
//     [15:0]    = I sample 0    [31:16]   = Q sample 0
//     [47:32]   = I sample 1    [63:48]   = Q sample 1
//
//   m_axis_data_tdata packing (2 data paths, 1 SPC, 34 bits each padded to
//   40-bit boundaries):
...
module fir_dec3_2spc_wrapper (
  ...
  output logic [79:0] m_axis_data_tdata,
  output logic        m_axis_data_tvalid,
  output logic        m_axis_data_tuser
);

  fir_dec3_2spc fir_dec3_2spc_i (
    .aclk               (aclk),
    .aresetn            (aresetn),
...
endmodule : fir_dec3_2spc_wrapper
