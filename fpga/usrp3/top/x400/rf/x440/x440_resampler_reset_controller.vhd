...
--
-- Module: x440_resampler_reset_controller
--
-- Description:
--
--   Generate reset pulses for the RX/TX resamplers in the data clock domain.
--   Resets are triggered either by the incoming pulse from the rf_reset_controller
--   (dAdcResetPulse / dDacResetPulse, already in DataClk domain) or by a software-initiated trigger
--   (cSoftwareControl, in ConfigClk domain, CDC'd internally).
--
...
entity x440_resampler_reset_controller is
  port(
    -- Clocks
    ...
    -- ADC Resets
    dRxResamplerResetPulse : out std_logic;

    -- DAC Resets
    dTxResamplerResetPulse : out std_logic;

    -- SW Control
    -- Control to initiate resets to RFDC.
    cSoftwareControl   : in  std_logic_vector(31 downto 0)
  );
end x440_resampler_reset_controller;


architecture RTL of x440_resampler_reset_controller is
...
begin
  ...
  begin
    if rising_edge(DataClk) then
      dRxResamplerResetPulse <= dAdcResetPulse or dTriggerAdcReset;
      dTxResamplerResetPulse <= dDacResetPulse or dTriggerDacReset;
    end if;
