...
// Description:
//
//   RX decimate-by-3 module. Wraps the FIR Filter IP (4 SPC input),
//   rounds/clips the full-precision 34-bit FIR output back to 16-bit per path,
//   and outputs 4 IQ samples per valid in the data_clk domain.
//
...
module rx_dec3 (
  ...
  // Input DATA channel (AXI4-Stream slave, 4 SPC @ rfdc_clk)
  input  logic [127:0] adc_data_in_tdata,
  input  logic         adc_data_in_tvalid,
  output logic         adc_data_in_tready,

  // Output DATA channel (4 SPC @ data_clk)
  output logic [127:0] adc_data_out_tdata,
  ...
  // Constants
  //---------------------------------------------------------------------------
  localparam int NUM_PATHS      = 4;   // Output data paths from FIR (2x I and Q)
  ...
  fir_dec3_4spc_wrapper fir_dec3_4spc_wrapper_i (
    .aclk               (rfdc_clk),
    .aresetn            (resetn_rclk),
  ...
  // CDC
  //---------------------------------------------------------------------------
  // reuse the 2 SPC to 4 SPC gearbox to cross from rfdc_clk (3x) to data_clk (1x)
  localparam int CDC_WIDTH = 24;
  logic [NUM_PATHS-1:0][CDC_WIDTH-1:0]   cdc_tdata_in;   // 2 IQ samples
  logic [NUM_PATHS*2-1:0][CDC_WIDTH-1:0] cdc_tdata_out;  // 4 IQ samples

  // extend input data to match CDC module width
  always_comb begin
    for (int i = 0; i < NUM_PATHS; i++) begin
      cdc_tdata_in[i] = rc_tdata[i];
    end
  end
  ...
  always_comb begin
    for (int i = 0; i < NUM_PATHS*2; i++) begin
      adc_data_out_tdata[i*DATA_WIDTH +: DATA_WIDTH] = cdc_tdata_out[i][DATA_WIDTH-1:0];
    end
  end
