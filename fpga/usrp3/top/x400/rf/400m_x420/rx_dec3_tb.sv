...
module rx_dec3_tb;
    ...
    // Coefficients from ip/fir_dec3_4spc/fir_dec3_4spc.xci (PARAM_VALUE.CoefficientVector)
    ...
    // Input: 128 bits @ rfdc_clk
    AxiStreamIf  #(.DATA_WIDTH(128), .TUSER(0), .TKEEP(0), .TLAST(1)) m_axis_if
      (.clk(rfdc_clk), .rst(reset));
    // Output: 128 bits @ data_clk
    AxiStreamIf  #(.DATA_WIDTH(128), .TUSER(0), .TKEEP(0), .TLAST(1)) s_axis_if
      (.clk(data_clk), .rst(reset));
    ...
    AxiStreamIf  #(.DATA_WIDTH(160), .TUSER(0), .TKEEP(0), .TLAST(1)) fir_s_axis_if
      (.clk(rfdc_clk), .rst(reset));
    AxiStreamBfm #(.DATA_WIDTH(160), .TUSER(0), .TKEEP(0), .TLAST(1)) fir_stream_bfm;
    ...
    typedef AxiStreamBfm #(.DATA_WIDTH(160), .TUSER(0), .TKEEP(0), .TLAST(1))
    ...
    initial begin : tb_main
        ...
        begin
          localparam int NUM_SAMPLES = 120;
          // Observed deterministic FIR phase order at output is
          // run0->phase2, run1->phase1, run2->phase0.
          localparam int PHASE_MAP [0:2] = '{2, 1, 0};
          int reconstructed[NUM_SAMPLES] = '{default:0};

          for (int i = 0; i < 3; i++) begin
            packet_in = new();
            // 120 samples with 4 SPC = 30 packets
            for (int j = 0; j < NUM_SAMPLES/4; j++) begin
              if (j==0) begin
                automatic logic [127:0] impulse = '0;
                impulse[32*i +: 16] = 16'h1;
            ...
            // Keep only even beats, where valid samples appear on I0.
            for (int p = 0; p < NUM_SAMPLES/3/2; p++) begin
              logic signed [33:0] sample;
              int dst;
              fir_stream_bfm.get(fir_packet_out);
              for (int s=0; s < 2; s++) begin
                sample = fir_packet_out.data[0][s*80 +: 34];
                dst = p*3*2 + PHASE_MAP[i] + s*3;
                if (dst < NUM_SAMPLES) begin
          ...
          for (int idx = NUM_FIR_TAPS; idx < NUM_SAMPLES; idx++) begin
        ...
        begin
          ...
          for (int s = 0; s < 4; s++) begin
            ...
            // Wait for all expected output transfers to arrive.
            while (axi_stream_bfm.num_received() < EXPECTED_OUT_TRANSFERS) begin
