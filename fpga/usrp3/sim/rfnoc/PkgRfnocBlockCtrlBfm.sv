...
package PkgRfnocBlockCtrlBfm;
  ...
  class RfnocBlockCtrlBfmCtrlOnly;
    ...
    // Send a write request packet on the AXIS-Ctrl interface and get the
    // response.
    //
    ...
    task reg_read64(
      ...
      assert (running) else begin
        $fatal(1, "Cannot call reg_read64 until RfnocBlockCtrlBfm is running");
      end

      ctrl.reg_read64(addr, word, timestamp);
    endtask : reg_read64
    ...
    task reg_write64(
      ...
      assert (running) else begin
        $fatal(1, "Cannot call reg_write64 until RfnocBlockCtrlBfm is running");
      end

      ctrl.reg_write64(addr, word, timestamp);
    endtask : reg_write64


    // Send a block write request on the AXIS-Ctrl interface and get the
    // response. Writes num_data words to consecutive addresses starting at
    // addr (addr, addr+4, addr+8, ...).
    //
    ...
    task block_write(
      ...
      assert (running) else begin
        $fatal(1, "Cannot call block_write until RfnocBlockCtrlBfm is running");
      end

      ctrl.block_write(addr, data, timestamp);
    endtask : block_write


    // Send a block read request on the AXIS-Ctrl interface and get the
    // response. Reads num_words words from consecutive addresses starting at
    // addr (addr, addr+4, addr+8, ...).
    //
    ...
    task block_read(
      ...
      assert (running) else begin
        $fatal(1, "Cannot call block_read until RfnocBlockCtrlBfm is running");
      end

      ctrl.block_read(addr, num_words, data, timestamp);
    endtask : block_read


    // Send a burst write request on the AXIS-Ctrl interface and get the
    // response. Writes num_data words to the same address (no address
    // increment).
    //
    ...
    task burst_write(
      ...
      assert (running) else begin
        $fatal(1, "Cannot call burst_write until RfnocBlockCtrlBfm is running");
      end

      ctrl.burst_write(addr, data, timestamp);
    endtask : burst_write


    // Send a burst read request on the AXIS-Ctrl interface and get the
    // response. Reads num_words words from the same address (no address
    // increment).
    //
    ...
    task burst_read(
      ...
      assert (running) else begin
        $fatal(1, "Cannot call burst_read until RfnocBlockCtrlBfm is running");
      end

      ctrl.burst_read(addr, num_words, data, timestamp);
    endtask : burst_read
