...
//   i.e. changes to SPP will not take effect until the burst.
//
...
// Parameters:
//
//   WIDTH                       : Data AXI stream bus width
//   CHDR_W                      : CHDR data bus width
...
#(
  parameter WIDTH                       = 32,
  parameter CHDR_W                      = 64,
  parameter SIDEBAND_FWD_FIFO_SIZE_LOG2 = 5 // 0 is not recommended!
)(
  // Clock/Reset
  input  wire              clk,
  input  wire              reset,
  ...
  output wire [WIDTH-1:0]  m_axis_user_tdata,
  output wire              m_axis_user_tkeep,
  ...
  input  wire [WIDTH-1:0]  s_axis_user_tdata,
  input  wire              s_axis_user_tkeep,
  ...
  assign m_axis_user_tvalid    = s_axis_tvalid & ((in_state == S_IN_FWD_SIDEBAND) ? sideband_fwd_fifo_in_tready : 1'b1);
  assign s_axis_tready         = m_axis_user_tready & ((in_state == S_IN_FWD_SIDEBAND) ? sideband_fwd_fifo_in_tready : 1'b1);
  ...
  always @(posedge clk) begin
    case (out_state)
      ...
      S_OUT_WAIT_FOR_FWD_FIFO : begin
        ...
        out_pkt_size <= (spp > 0) ? (WIDTH/8)*spp : fifo_out_tlength;
        out_pkt_cnt  <= (WIDTH/8);
      ...
      S_OUT_PACKETIZE : begin
        if (s_axis_user_tvalid & s_axis_user_tready) begin
          out_pkt_cnt <= out_pkt_cnt + (WIDTH/8);
